...
| 1000 | `to <dns> iif lo lookup <table>` | the watchdog's DNS probe | adds one for the length of one lookup and removes it; each run first sweeps any left by a run that was killed | aims the probe through the tunnel it is testing (ID-078) |
