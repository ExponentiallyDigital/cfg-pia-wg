...
| Aspect | Value |
| --- | --- |
| Paths | `/jffs/scripts/watchdog_wgcN.sh`, log `/tmp/watchdog_wgcN.log`, status `/tmp/watchdog_last_ping_success_wgcN`, backoff `/tmp/watchdog_backoff_wgcN`, CA cache `/jffs/cfg-pia-wg/pia_ca.rsa.4096.crt` (`kPiaCaCertPath`; the script `mkdir -p`s the directory before downloading, since Merlin has no reason to have created it) |
