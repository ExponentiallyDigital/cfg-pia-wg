...
- **CHANGELOG.md entries are flat and short, and their ORDER carries meaning.** Every line item is a standalone top-level `- ` bullet that reads correctly on its own, in the existing `- FIX:` / `- CHG:` / `- ADD:` / `- TST:` / `- DOC:` / `- INF:` style, kept to a sentence or two - detail belongs in code comments or `ARCHITECTURE.md`. Put the most important change FIRST: the release action no longer sorts a block, so what ships as the GitHub release note is the block exactly as written. (It used to sort alphabetically, which is why this rule once said sub-bullets were forbidden. They are not, as of build 441 - but a flat list still reads better.)
...
| `device_assignment_screen.dart` | The assignment screen: default-connection panel, device list, per-device picker, staged changes (held on the session), and the apply confirmation with its tunnel warnings; a note under any picker whose tunnel is not running says where the traffic goes. |
...
- **HOUSE STYLE: every button is bordered and unfilled, with the border and the label in ONE colour.** Decided 2026-09-13. Two screens are exempt and nothing else is: the **main menu** keeps its filled buttons, and the **paywall** keeps its filled purchase button and its borderless restore and NOT NOW links, because it is the one screen built to sell rather than to operate. Do not "fix" either exemption, and do not add a fill anywhere else.
