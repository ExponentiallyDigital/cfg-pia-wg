...
| `entitlement.dart` | `Entitlement.isUnlocked` - the seam RevenueCat will fill. Returns true today, so nothing is withheld while the app has no way to take money. Mirrored on `SessionController.isUnlocked`, which notifies, because a gated control has to rebuild the moment a purchase completes. **The gating rule: created or changed is paid, removed is free.** CREATE, ENABLE, EDIT, watchdog enable and device APPLY are gated; DISABLE, DELETE and VIEW LOG are not. On the slots screen the entitlement check runs BEFORE the stock dependency probe - see `test/widgets/router_slots_screen_test.dart`, which exists only to pin that order. |
