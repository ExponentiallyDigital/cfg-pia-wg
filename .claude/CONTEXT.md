...
| Watchdog start / stop of the tunnel | `service start_wgc N` / `stop_wgc N` | `nvram set vpnc_unit=<row>` + `service restart_vpnc` / `stop_vpnc` (`RouterSlotService.runVpncService`) — the same calls MANAGE makes. **Unproven:** that the Merlin commands are inert on stock. The watchdog script's own reconfigure still calls `stop_wgc` / `start_wgc` / `restart_vpnrouting0` on stock, and the only hardware log of it (2026-09-10) was taken while the router's service queue was stuck. Tracked as BACKLOG ID-070. |
