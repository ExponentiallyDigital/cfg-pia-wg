...
- **Never send a Dart constant to the router escaped as text.** A shell command string containing `\$kSomething` asks the router's shell to expand a variable it has never heard of; it expands to nothing, and a file test built that way always fails. Interpolate the Dart value, or use a path helper from `firmware.dart` such as `watchdogScriptPath`. `test/unit/no_escaped_constants_test.dart` fails on it.
...
- **Andrew runs the commits, and every commit is a build.** Do not run `git commit` unless he asks for it in that message. Every commit follows the before-and-after steps at the top of `CHANGELOG.md`: the release block's summary replaces `in progress` before the commit, and the next block and the `pubspec.yaml` bump follow it. Burning build numbers on small commits is accepted.
- **Say what a CHANGELOG WIP edit changed.** Say which lines were removed, moved or reworded, and give the WIP bullet count before and after - these edits happen between commits, so git cannot show him.
- **CHANGELOG.md carries its own instructions at the top of the file, and they win over anything here.** They cover `ID-NNN` identifiers shared with BACKLOG, the prefix codes, bolding, ordering (Pending and WIP by importance; each release block chronological, so a completed item is added at the bottom), moving a completed WIP item into the current block, and who opens the next block and bumps `pubspec.yaml`. Read them before editing the file. Its structure must not change: GitHub Actions read it, and the release action does not sort a block, so what ships as the GitHub release note is the block exactly as written.
- **A release block can open with prose, and that prose becomes the top of the GitHub release note.** Anything between the `<date> v<x.y.z> build <n> - <title>` line and the next version header is published verbatim, links and callouts included. Write the release note there, under the date line, rather than by hand on GitHub: `softprops/action-gh-release` REPLACES the body on every run, so a hand-edited note is one workflow re-run away from being wiped. That happened on 2026-09-12 and the text was unrecoverable - GitHub keeps no history of release bodies.
- **The Play Store "what's new" comes from a ```` ```play ```` fence in the release block.** One CHANGELOG block, two audiences: everything outside the fence becomes the GitHub release body, and the fence itself becomes the Play note. They cannot share text - Play allows **500 Unicode characters** per language, renders no markdown, and does not make a URL tappable, so a developer preamble pasted there shows literal asterisks and a dead link cut off mid-word. Write the fence FIRST, under the date line, while the release is fresh.
- **The build fails if the Play note exceeds 500 characters**, and warns if a release block has no fence at all, in which case Play falls back to a link to the hosted changelog. Both are caught in CI rather than in Play Console. Rendered on GitHub the fence shows as a plain monospace block, which is an honest preview of what the Play listing will look like. The rules, and what the rest of the `play-store/` folder is for, are in [`play-store/README.md`](../play-store/README.md).
- **Never record Andrew's real network identifiers in a file the repo tracks.** No LAN or WAN IP addresses, no hostnames, no DDNS names, no MAC addresses, no router login names, no PIA username, no router model numbers. PIA region names used in testing (`us_alabama`, `aus_perth`) are not identifiers and may be named. He shares these freely in chat to get a problem solved; that is not consent to publish them. When hardware output has to be quoted in a doc, plan or test, substitute invented values - `my-router.asuscomm.com`, `192.168.1.20`-`192.168.1.25`, `AA:BB:CC:DD:EE:FF` - and say in the document that they are invented, so a later reader does not treat them as real and reinstate the originals. Verbatim session logs go under `.claude/testing/`, which is `.gitignore`d for exactly this reason. There are no exceptions: `kDefaultRouterIp` was one until build 410 and is now the ASUS factory address `192.168.50.1`, pinned by a test.
- **The app never changes the DNS settings the user made in the router's WebUI.** WAN DNS, DNS-over-TLS servers, DNSSEC, rebind protection and the rest are the user's. A VPN the app creates uses the DNS server(s) the user supplied for that VPN in the app, and nothing else. Measured 2026-09-14: a slot's DNS field (`wgcN_dns`) leaves the router's resolver configuration unchanged - `/tmp/resolv.dnsmasq` was identical with a tunnel up, disabled and deleted. The firmware does add a `from all to <slot DNS> iif lo` routing rule per tunnel, so when a slot's DNS matches the router's own DNS-over-TLS servers the router's own lookups leave through a tunnel: a firmware consequence of the user's two settings, recorded in BACKLOG AN-2026-09-13_001, not something the app changes.
...
- **`README.md` is Andrew's, and it is off limits.** Do not edit it, not even a typo, a broken link or a stale number spotted while working on something else - say so in the reply and leave the file alone. He rewrites it himself, at length, and an unrequested edit lands in the middle of his draft. When he DOES ask for something to go in, **show the exact proposed text in the reply and wait for his go-ahead before writing the file**; never write it and show it afterwards. This is the root `README.md` only - `play-store/README.md` and other subfolder READMEs follow the ordinary rules.
- **Markdown prose is written in Andrew's voice.** `.claude/testing/voice-guide.md` describes how he writes: Australian English, the reader's question asked and then answered, measured facts with dates, no corporate filler, and a register chosen per document - reference voice for ARCHITECTURE and TESTING, the restrained blog voice for README. Read it before writing or rewriting prose in any tracked Markdown file. CONTEXT.md itself is the exception: it is a rules file for Claude, and its clipped register is deliberate. **The guide is never committed.** It lives under `.claude/testing/`, which `.gitignore` covers, and it is the only document that connects Andrew to a pseudonymous site; `test/unit/no_lan_identifiers_test.dart` fails the build if git ever starts tracking it (ID-119).
...
| SSH lifetime | **One connection per app session**, shared by every action - `RouterSession` in `router_session.dart`, owned by `SessionController`. Opened lazily, reused, reconnected once on a transport failure, and closed by `wipeAll()`, or once the app has been in the background for `kBackgroundSessionGrace` (5 minutes; coming back sooner cancels the close). Until 406 it was a fresh `SSHClient` per action, closed in a `finally`; **an action must never call `close()` now** - it would pull the connection out from under the next one, and a source scan in `test/unit/router_session_test.dart` fails the build if one appears. |
| Licence header | Every `lib/` file opens with the GPL-v3 header block + `Copyright (C) 2026 Andrew Newbury.` Keep it on new files. |
| Colours | Never inline a hex colour in a screen; use the constants in `lib/app_colors.dart`. Anything named after a destination - a menu row, a drawer row, a screen heading, a slot action - takes its colour from `kDestinationColours` / `kSlotActionColours` in that file, not from its own copy (ID-112). |

## 2. Snapshot

The app opens on a main menu (`MainMenuScreen`) offering all eight destinations plus EXIT; a hamburger drawer rendered *above* the Navigator offers the same destinations under the same names. Screen 1 generates a standalone PIA WireGuard config (region → credentials → `GENERATE CONFIG`) with a 60-second clipboard auto-clear and SHARE/SAVE. Screens 2 and 3 SSH into an ASUS router and then push a shared full-screen `wgc1..wgc5` slot list: *manage* mode does CREATE / ENABLE / EDIT / DISABLE / DELETE of WireGuard slots; *watchdog* mode does CREATE-EDIT / DELETE / VIEW ROUTER WATCHDOG LOG and deploys a router-side POSIX-sh watchdog that re-negotiates PIA on ping failure. **Both Merlin and stock ASUS firmware are supported** — the firmware is detected once per session on entry to either router screen and every router command branches on it (§4.13). Screen 4 pins individual LAN devices to a tunnel (stock only, §4.14) and screen 5 shows the in-memory app log. Three more screens page through the router's syslog, remove things - the router uninstall lives on SETTINGS - and carry the build information; they were drawer-only once, and every destination now sits on the main menu as well (ID-102). All credentials and generated config are volatile — held only in `SessionController` and wiped on every exit path — though PIA and SMTP credentials *are* written to router NVRAM in plaintext when a watchdog is deployed.
...
| `watchdog_dialog.dart` | `WatchdogDialog` — the watchdog CREATE/EDIT form. Its `SAVE & DEPLOY` validates, takes the region from the form, WAN-pings both targets (warn-only), then calls `deployWatchdog`. **This is the only path that brings a watchdog up.** |
...
| `review_service.dart` | `openPlayStoreReview()` - opens `kPlayStoreListingUrl` with `url_launcher`'s `launchUrl(..., mode: LaunchMode.externalApplication)`, so the Play Store app takes it rather than an in-app browser tab (the manifest `<queries>` already covers an https VIEW). Returns false when nothing could be opened, and the caller logs that. **Deliberately does NOT ask for Play's in-app rating card** - see §4.8.3. No plugin since ID-042: `in_app_review` applied the Kotlin Gradle plugin unconditionally, which fails the build under built-in Kotlin. Tests mock url_launcher's channel (`installUrlLauncherMock` in `test/unit/review_service_test.dart`), and a test holds the listing id to `applicationId` in `build.gradle.kts`. |
...
| `router_service_queue.dart` | `RcServiceState`, `parseRcService`, `RouterServiceQueue`, `RouterServiceWedgedException`. Survives the router's own `notify_rc` queue: a service that never finishes leaves `rc_service` set and every later event is discarded after a 15-second wait. Guards every service call, before and after. A marker counts as a ghost only once it has sat UNCHANGED for ten seconds: `rc_service_pid` names `notify_rc`, which exits as soon as it has queued the job, so pid-liveness says nothing about the service. ARCHITECTURE.md "The router's service queue, and how it wedges". |
| `device_assignment.dart` | Pure layer for VPN Fusion device policy - no SSH. `DevicePolicy`, `parseDevicePolicyList` / `serialiseDevicePolicyList`, `setDevicePolicy`, `assignedIndexFor`, `staleRuleTables`, `LanDevice`, `buildDeviceList` and the four-source join. Also the APPLY tunnel check - `TunnelHealth`, `parseTunnelHealth`, `tunnelWarning` - and `actualExitIndex`, where a device's traffic really leaves when its tunnel is not running. |
| `device_assignment_service.dart` | The I/O half: one marker-delimited read of eight sources, and an `apply()` that re-reads and refuses on conflict before writing anything. Owns the eleven-probe default-connection sequence and the stale `ip rule` sweep, and `tunnelHealth`: up state plus latest handshake against the router clock, one round trip. |
| `fail_closed_guard.dart` | `kGuardScript` (`/jffs/cfg-pia-wg/guard.sh`) and `FailClosedGuard` (`ensure` / `install` / `clear`). Two rules per device pinned to a WireGuard slot - `90: lookup <table> suppress_prefixlength 0`, `91: blackhole` - reconciled from `vpnc_dev_policy_list` on every run. Called after APPLY, before DISABLE, after DELETE's release, by every watchdog check, at boot from S50, and by UPDATE WATCHDOG VERSION; UNINSTALL clears it first. `ensure` never throws: a guard that cannot be put in place is a warning in the app log, not a failed action. ARCHITECTURE 6.8.10. |
...
| `entitlement.dart` | `Entitlement.isUnlocked`, backed by RevenueCat. A build with no store key (local or self-built) can take no money and withholds nothing. A store build is locked until RevenueCat reports the entitlement: a first run that cannot reach the store stays locked, and a returning customer is covered by the SDK's own cache. Mirrored on `SessionController.isUnlocked`, which notifies, because a gated control has to rebuild the moment a purchase completes. **The gating rule: created or changed is paid, removed is free.** Gated: MANAGE CREATE, ENABLE and EDIT; WATCHDOG CREATE/EDIT and ENABLE; device APPLY; ABOUT REDEPLOY TO UPDATE VERSION; SETTINGS MAX ACTIVE VPNS. Free: DISABLE, DELETE, VIEW LOG, and the rest of SETTINGS. On the slots screen the entitlement check runs BEFORE the stock dependency probe - see `test/widgets/router_slots_screen_test.dart`, which exists only to pin that order. |
...
| File | Role |
| --- | --- |
| `main_menu_screen.dart` | Nine left-aligned rows (`_MenuRow`, ID-031): every destination from `AppDrawer.destinations`, in that order - icon from `destinationIcon`, title, chevron - then EXIT (`Key('menu_close_app')`), red with `kExitIcon`, no chevron and a little space above it. Each key is `menu_` plus the route name. The list is capped at `kMenuMaxWidth` (520) and centred, through `AppScaffold.maxContentWidth`, so a tablet does not stretch the rows into bars. Directly beneath them, with no gap between the two, a "(?) how to use this app" link (`Key('menu_help')`, `kHelpUrl` -> README section 5) and a "(*) add a Play Store app review" link (`Key('menu_review')` -> `review_service.dart`), both left-aligned with the rows; spare height goes below them. The PayPal/Patreon donation block was removed when the app gained a price - one ask per screen. `PopScope(canPop: false)` routes the Android back key to `confirmAndExit`. |
...
| `log_screen.dart` | `ListenableBuilder` over the controller → `LogPanel` + `CLEAR LOG`, under an APP LOG heading (`Key('app_log_heading')`) placed and styled like the watchdog log's (ID-033). |
| `slot_params_editor.dart` | Modal editor for the 17 per-slot NVRAM values (spec 3.3). |
| `settings_screen.dart` | In order: REBOOT ROUTER, FORGET ROUTER IP, REMOVE CACHED PIA CERT, UNINSTALL FEATURES DEPLOYED TO ROUTER, and RESTORE PURCHASE on a store build, followed by a further row described in the code. On the main menu and in the drawer. |
| `router_log_screen.dart` | `/tmp/syslog.log`, newest 32 KB first, paging backwards on demand and continuing into the rotated file. COPY / REFRESH / HOME. Headed ROUTER LOG (`Key('router_log_heading')`, ID-033). `routerLogLineColour` picks each line's colour: red for an error the app or the watchdog wrote, teal for the app, lavender (`kWatchdogText`) for the watchdog - not amber, which means a warning elsewhere in the app - and plain for the firmware (ID-034). |
| `about_screen.dart` | Build info is ONE `Text.rich` with `\n` between rows, not a widget per row — `SelectionArea` joins separate widgets with no separator, so a row-per-widget layout copies as one run-on line. `COPY BUILD INFO` (`Key('about_copy_build_info')`) and `CREATE GITHUB ISSUE` (`Key('about_create_issue')`) share a `Wrap`. The block's rows start Watchdog script, then Router firmware once the router has been read (type and version, `routerFirmwareStatus`), then License status (`licenceStatus`: licensed, unlicenced, or homegrown for a keyless build), then the build facts. When the deployed script's version differs from the app's, its value is amber and REDEPLOY TO UPDATE VERSION (`Key('about_redeploy_scripts')`) appears under the block: paid, and it rewrites only script files already on the router, via `RouterWatchdog.redeployScripts`. The latter opens `bugReportUrl()`: `<repo>/issues/new` with `title` and `body` prefilled, reproducing the headings of `.github/ISSUE_TEMPLATE/bug_report.md` (GitHub honours a template **or** a `body`, never both) and filling Environment with `asPlainText` plus the detected firmware. A test fails if the template's headings drift. Both buttons take their own line at phone width - CREATE GITHUB ISSUE alone is 282px of a ~320px body - which is what the `Wrap` is for. The screen also reads the deployed watchdog script version and the reconfigure history over SSH (`aboutRouterFacts`, one round trip); with no router credentials in the session it asks for them inline via the shared `SshCredsDialog` and writes them back, because ABOUT is reachable without ever visiting a router screen. Takes a `testClientFactory` like the router screens. COPY writes `_BuildInfoBlock.asPlainText` via `SessionController.copyToClipboard(armAutoClear: false)` - not a secret, so it arms no countdown and stands down one an earlier config copy left running. This is the screen's only `SessionScope` dependency. Whole body wrapped in one `SelectionArea` (drag or Select all copies build info + links + licence together), so its children are plain `Text`, not `SelectableText`. Build provenance from `loadBuildInfo()`, 6 project links, an open-source `_LicensesDialog` (custom dark replacement for `showLicensePage`) whose entries go through `groupLicensesByPackage` — one heading per package, not per `LicenseEntry`, since the registry emits an entry per licence text naming every package it covers, and the full GPL text. The dialog keeps notices as `LicenseParagraph` lists and renders each through `_LicenceParagraph` (`centeredIndent` ⇒ centred + bold, else `16.0 * indent` padding), and carries its own `SelectionArea` — the screen's region does not reach into a dialog route. Hard line breaks inside a notice are already gone before we see it (`LicenseEntryWithLineBreaks` joins a paragraph's lines with a space), so expat's ASCII art reads as run-on text here exactly as it does in Flutter's own licence page. |
...
| `app_drawer.dart` | `screenForDestination()`, `navigateToDestination()` (no-op on current; **pushes**, growing the stack by design), `closeApp()`, `confirmAndExit()`, `AppDrawer`, and `destinationIcon()` / `kExitIcon` - the one place each destination's icon is chosen, shared by the drawer and the main menu (ID-031). |
| `slot_modal.dart` | `SlotModalMode` enum + `SlotModal` (slot list - each row one `wgcN:region` line, ID-035 - badges, mode-dependent button set, all router actions) + `_PiaCredsDialog` + `_PingTargetsDialog`. Despite the file name it is a full-screen page since 418, wrapping `AppScaffold` in a `Stack` so the processing overlay covers HOME as well. |
...
| `paywall.dart` | `PaywallOffer` (the store's own price string, `purchase`, `restore`), `Paywall.show` and `Pitch` (one sentence per gated action). A full page, not a dialog, so its content cannot outgrow its viewport. Opens CONTEXTUALLY - on tapping a control that would create or change something - never on launch or on entering a screen. Keys `paywall_buy`, `paywall_restore`, `paywall_close`. Every purchase and restore outcome goes to the app log as well as the screen, store errors are turned into sentences by `Entitlement.describeStoreError`, a pending payment is a warning rather than a failure, and restore's wording lives in `RestoreMessages`, shared with SETTINGS. |
...
| `device_assignment_screen.dart` | The assignment screen: default-connection panel, device list, per-device picker, staged changes (held on the session), and the apply confirmation with its tunnel warnings; a note under any picker whose tunnel is not running says where the traffic goes. Each device row reads `name - tags` (tags joined by ` \| `) over `IP MAC` in grey, the MAC alone when no address is known (ID-032). |
...
RouterSlotsScreen ──connect()──> SSHClient ──> RouterSlotService.fetchSlots()
        └─> SlotModal(mode)
              ...
              │            _enableManage → enableSlot → enableWatchdog if it was paused
              │            _editManage  → readSlotParams → SlotParamsEditor → writeSlotParams
              │            _disableManage (disableWatchdog: pause) / _deleteManage (stopWatchdog: remove)
...
- **HOUSE STYLE: one control, one size, on every screen.** `HOME` is full width and pinned at the bottom EVERYWHERE, including on screens whose body is capped by `maxContentWidth` - the cap is for content, not for chrome. A control that changes size between screens reads as an accident even when each screen looks fine on its own.
  - **The three LOG screens are the deliberate exception**: each carries a row of three bordered equal-width buttons, so no single button can be full width there. `COPY REFRESH HOME` on the router log, `COPY CLEAR HOME` on the app log, `COPY CLEAR CLOSE` on the watchdog log - the last closes because it is pushed from the slot list rather than reached from the menu. Do not "fix" any of them back.
  - All three also open scrolled to the NEWEST entry. The reason anyone opens a log is to see what just happened; opening at the top means scrolling past a session to reach it.
- **HOUSE STYLE: every button is bordered and unfilled, with the border and the label in ONE colour.** Decided 2026-09-13. One screen is exempt and nothing else is: the **paywall** keeps its filled purchase button and its borderless restore and NOT NOW links, because it is the one screen built to sell rather than to operate. Do not "fix" that exemption, and do not add a fill anywhere else. The **main menu** was exempt until ID-031 and now follows the style in a row layout of its own: each row has a `kBg` fill - the screen colour, so it reads as unfilled - with a teal border and label, and EXIT is red.
  - **Colour is the button's role, not its screen.** Teal (`kHighlight`) for an action. Red (`kError`) for anything that destroys, removes, discards, reboots or exits. Grey (`kMuted`) for a way out that changes nothing: CANCEL, CLOSE, NOT NOW.
  - **Disabled is `kHint` for both border and label.** A greyed-out action must never look like a live CANCEL, which is why disabled is darker than the dismiss grey rather than the same colour.
...
| `routerIp`, `sshUsername`, `sshPassword` | Volatile. The form prefills via `routerIpPrefill`: session value, then `rememberedRouterIp`, then `kDefaultRouterIp` (`192.168.50.1:22`, the ASUS factory address). **Only the address is ever prefilled** - `sshUsername` and `sshPassword` start empty and are never persisted. |
...
- **Only after a proven connect.** `rememberRouterIp` is called only after the SSH work succeeds, from every screen that asks for router credentials: `router_slots_screen._onConnect`, `router_log_screen`, `settings_screen` and `about_screen`. Same rule as `TextInput.finishAutofillContext()` - never persist an unproven value.
...
| watchdog | `slot_wd_enable` | ENABLE | `hasDesc && wdConfigured && !wdActive` |
| watchdog | `slot_wd_disable` | DISABLE | `hasDesc && wdActive` |
| watchdog | `slot_delete` | DELETE | `hasDesc` |
| watchdog | `slot_view_log` | VIEW ROUTER WATCHDOG LOG | `hasDesc && (wdActive \|\| wdConfigured)` |

**The watchdog asks three questions, in cost order** (script, `router_watchdog.dart`): is the interface up, has it handshaked within 300 s, and - since ID-078 - does the slot's own first DNS server answer through this tunnel. The third runs only when the first two pass, needs `wgcN_dns` set (skipped otherwise), verifies with `ip route get` that its lookup really leaves through this interface before believing the answer, and needs two consecutive failures to act. Its temporary `ip rule` is marked by priority 1000 and removed on every path. A rebuild waits for a handshake before reporting success (ID-063), and on stock restarts through VPN Fusion by row index, as the app does (ID-070). Its own lookups go over DoH with a hostname URL and `--resolve`, because ASUS's curl refuses IP-literal URLs (ID-076, ARCHITECTURE section 2), and the SMTP hostname is resolved the same way before the mailer runs (ID-077).

**MANAGE DISABLE pauses a watchdog and MANAGE ENABLE resumes one** (ID-095): a watchdog left running would rebuild the tunnel the user has just stopped, and tearing it down would take the settings - and, on the last watchdog on the router, the PIA credentials - with it. **Watchdog ENABLE and DISABLE act on the schedule, not the tunnel.** DISABLE (`disableWatchdog`) removes the slot's two `cru` jobs and its boot-persistence lines and keeps the script, the NVRAM settings and the tunnel, so the slot shows PAUSED. ENABLE puts the schedule back at the interval stored on the router. Watchdog DELETE (`stopWatchdog`, then `deleteSlot`) removes all of it and the VPN too. VIEW LOG stays available while paused, because the log of the run that prompted a DISABLE is what you want to read afterwards. All buttons are disabled while `_processing`.

Row badges: `● ACTIVE` (up AND answered - `activeSlots.contains(n) && answeringSlots.contains(n)`), `● UP, NO ANSWER` (up with no handshake inside `kAnsweringWithinSeconds`, amber - ID-123), `⚑ KILL SWITCH` (`enforce==1`, amber), `◆ WATCHDOG ACTIVE` (`watchdogActive` - a cron entry exists), `⏸ WATCHDOG PAUSED` (`watchdogConfigured && !watchdogActive` on a non-empty slot: DISABLE removed the schedule and kept the settings; muted grey, since nothing is running), `✉ EMAIL ALERTING` (only alongside WATCHDOG ACTIVE). The two watchdog badges are mutually exclusive.
...
`RouterSlots.activeSlots` is a **`Set<int>`** built from *all* matches of `wgc(\d)` in `wg show interfaces` — more than one tunnel can be up at once (stock `vpnc_max_conn`), and the previous `firstMatch` silently badged an arbitrary one of them. It is independent of `SlotInfo.enabled` (the NVRAM / `vpnc_clientlist` flag): the set means *the interface is up*, the flag means *it is configured on*. They can legitimately disagree while an action is in flight.

**`RouterSlots.answeringSlots`** is the subset whose server has answered within `kAnsweringWithinSeconds` (300 s, the watchdog's own window), from `parseHandshakeAges(kHandshakeAgesCommand)` — one `wg show all latest-handshakes` plus the router's own `date +%s`, so ages are never measured against the phone's clock. A `0` from `wg` means *never handshaked* and is dropped rather than read as "just now", because that is exactly what an expired registration leaves behind. Up-but-unanswered is badged amber rather than left bare: a missing badge reads as "not enabled" (ID-123). Best-effort — if the command fails, every up slot is treated as answering, which is what the badge did before this existed.

**A stop that does not take the interface down is reported** (ID-124). `_awaitInterfaceDown` polls `verifyMaxAttempts` × `verifyPollInterval` and returns false rather than throwing; DELETE carries on clearing the configuration, while `disableSlot` returns that false to MANAGE DISABLE, which says so in a dialog. Nothing self-refreshes the badge afterwards — that is BACKLOG ID-122.
...
**`WatchdogDialog` fields:** region (a `RegionRow` pre-filled with the slot's own region and browsable, the same row STANDALONE has), check interval (min, default 5), primary IP (8.8.8.8), secondary IP (1.1.1.1), PIA username/password (pre-filled from session, mirrored back on every exit path via `_rememberPiaCreds`), and — behind the `Enable email alerts` switch — From, To, Subject (`cfg-pia-wg alert`), SMTP server `host:port`, SMTP username/password, plus `TEST EMAIL`.
...
**SAVE flow (`WatchdogDialog._save`):** jq gate and, on stock, the Download Master boot-directory gate → `validate()` → a region is required; when the watchdog is not currently enabled or the region has changed, the region is checked against PIA's list, then confirm-overwrite (when the slot is non-empty; when the region changes it says the tunnel is rebuilt on it, is down meanwhile, and assigned devices use the default connection) → WAN-ping both targets (`pingHostViaWan`, warn-only, "The settings will still be saved.") → `deployWatchdog(cfg, desc)` → if the slot was empty, `enableVpnSlot` → pop.

**`RouterWatchdog.deployWatchdog` — the order is load-bearing:**

1. `enableJffsScripts` (Merlin: `jffs2_scripts=1`, `jffs2_on=1`; stock: `mkdir -p /jffs/cfg-pia-wg`)
2. `_writeWatchdogNvram` (per-slot `wgcN_wd_*` + global PIA creds + optional `wgcN_desc`) + commit
3. `enableVpnSlot` — with `alreadyUp` when the slot's interface is up and the deploy keeps its region (both read before step 2 rewrites `wgcN_desc`): the enable flags and the clientlist row are still written, but no service call is made, because stock's `restart_vpnc` rebuilds VPN routing for every tunnel. A changed region is a rebuild instead: before step 2, `_clearForRebuild` stops a running tunnel (`RouterSlotService.disableSlot`, which waits for the interface to go) and blanks `priv`, `ppub` and `ep_addr` so nothing can bring the old server back; `enableVpnSlot(rebuilding: true)` then takes the full enable without waiting for an interface, and step 7's deploy run finds no working tunnel and builds one on the new region - the empty-slot shortcut's path. Without it the old server kept running under the new name (2026-09-14), because the deploy run leaves a tunnel with a recent handshake alone and a restart reloads the old keys.
4. heredoc-write `/jffs/cfg-pia-wg/watchdog_wgcN.sh` (`watchdogScriptPath`, 30 s timeout) + `chmod +x`
5. `cru a watchdog_wgcN "*/M * * * *" …` and `cru a watchdog_log_rotate_wgcN "0 0 * * *" …`
6. `_ensureServicesStart` — Merlin: recreate `/jffs/scripts/services-start` if absent, strip prior entries for this slot, append both `cru` lines. Stock: back up the original `S50downloadmaster` once, rewrite its REPLACEMENT block with both `cru` lines, stub out `S50asuslighttpd`, then run `S50downloadmaster start` to install the entries now
...
**`stopWatchdog`:** `cru d` both jobs, `rm` the script, `_removeCronPersistence` (Merlin: strip the `services-start` lines; stock: rewrite the `S50downloadmaster` block without this slot, keeping the file; `chmod 700` on both), `rm` `/tmp/watchdog_wgcN.log{,.old}`, `/tmp/watchdog_unsent_wgcN`, `/tmp/watchdog_last_ping_success_wgcN`, `/tmp/watchdog_backoff_wgcN`, `nvram unset` all 10 `wgcN_wd_*` keys, commit, then `_disableVpnSlot`. JFFS is left enabled. The **global** `cfg_pia_wg_user` / `cfg_pia_wg_password` are unset **only when no other slot still has a watchdog cron entry** (`_otherWatchdogsRemain`) — with concurrent watchdogs, clearing them early leaves the survivor unable to authenticate with PIA at its next renegotiation.
...
**Undeliverable alerts are counted, not resent.** `/tmp/watchdog_unsent_wgcN` holds a count and a timestamp; the next email that gets through reports them and clears it. An alert about lost connectivity can be undeliverable for the reason it fired. What broke DNS for the one seen on 2026-09-06 (`server misbehaving`) is not known, and it was not the default connection: traffic the router creates never follows it, the watchdog resolves through `/etc/resolv.conf` (the WAN's DNS first on the test router), and on 2026-09-14 a test email went out while a tunnel's encrypted DNS was blocked. A failed send therefore logs an `Email diag: resolv.conf [...]` line after `Email FAILED`: the resolvers, the interface to the first one (the interface only - the route names the WAN address and the log is quoted in emails), and what the SMTP host resolved to.

**Kill switch has three states, not two:** ON and `OFF - the kill switch is available but is not enabled` on Merlin, and on stock - which has none - the state of the app's fail-closed guard. Baked per firmware as `KILLSW_UP` / `KILLSW_FIXED` / `KILLSW_DOWN` - three tenses, because the same fact reads wrong in the wrong one: the tunnel is up on a deploy run, was down on a recovery, and is still down on a failure. A failure takes the still-down wording whether or not the run was a deploy.

**Stock reports the fail-closed guard (460, ID-198).** Before 460 the line said a pinned device "fell through to the default connection, so it stayed on a VPN" - wrong for the failure the watchdog exists for (a tunnel whose server stops answering blocks its devices outright) and, where it was true, a leak rather than a reassurance. The script now counts the enabled policy records naming its own index 6 and the guard's priority-90 rules naming the same table, and says one of three things: nothing is pinned here; the guard kept the N pinned devices off the internet; or the guard covers only some of them. When this tunnel is also the default connection it adds that devices only following the default are not covered. Counted from the router's rules, never assumed, so a guard a reboot cleared and nothing has put back yet is reported as missing.
...
**Teardown.** `wipeAll()` closes it, so every existing exit path already tears it down, and so does `AppLifecycleState.paused`, after a grace of `kBackgroundSessionGrace` (5 minutes, `router_session.dart`). An authenticated session held open behind a locked screen is a wider exposure than credentials sitting in memory, but closing it the instant the app is paused charged a reconnect for every glance at the router's WebUI. Coming back within the grace cancels the close; a wipe closes it at once. The next action reconnects.

### 4.8.3 The Play Store review link

The home-screen link opens the **store listing**, not Play's in-app rating card. 406 shipped `InAppReview.requestReview()` and 407 took it out: Play alone decides whether to draw that card, it is quota-limited per user, it never appears on a build Play did not install, and the API reports success either way - the plugin's own code says "the API does not indicate whether the user reviewed or if the dialog was shown". Debug and release both did nothing visible, and no fallback could fire because `isAvailable()` was true. Google's guidance is not to put that flow behind a button at all. `requestReview` would be right for an *unprompted* ask - after a successful watchdog deploy, say - but since ID-042 removed `in_app_review` that needs a review plugin back, one that supports built-in Kotlin.

The listing opens with `url_launcher` in `LaunchMode.externalApplication`. `platformDefault` is wrong here: for an https link it opens an in-app browser tab showing Play's web page, instead of handing the link to the Play Store app.
...
- `LogPanel` renders the log in blocks of `LogPanel.blockSize` (100) entries, each ONE `Text.rich` with the entries separated by `'\n'` and icons as `WidgetSpan`s, inside a `SelectionArea` (ID-004). A block whose first and last entry are unchanged is reused as the same widget, so a new line rebuilds only the last block; one paragraph for the whole log cost the log's length on every line. Never a widget per entry: `SelectionArea` joins separate widgets' text with no separator, so that layout copies as one run-on line - which is also why the blocks sit in a `SelectionContainer` whose `_BlockJoiningDelegate` puts a line break between them. Same trap as the About screen's build info block; a placeholder splits the paragraph into selectable fragments but adds no character to the copy.
- The log is capped at `kLogCapChars` (100,000 characters, `session_controller.dart`): past it the oldest lines go in one chunk, down to 90%, under a first line from `logTruncatedMessage` saying how many. The LOG screen listens to `SessionController.logChanges`, not the controller, so a clipboard countdown does not lay the log out again every second.
...
| Non-credentials on the device | Exactly one: the router LAN address, in `router.txt` under the app support directory, written only after a connect succeeds and clearable via SETTINGS -> FORGET ROUTER IP. See 4.2.1. Nothing else is persisted, and a test enforces that. |
...
**Stock precondition:** `jq` must exist at `/jffs/cfg-pia-wg/jq`; the watchdog screen additionally requires `/jffs/cfg-pia-wg/mailsend-go` (manage mode never sends email). A locked user is never checked for missing binaries: entitlement runs first. An unlocked user with a binary missing is offered an install first. INSTALL installs it and retries the connect; NOT NOW returns to the connect screen, and the decline is remembered for the session. A later visit in the same session shows `showMissingBinariesNotice` instead, whose INSTALL offers the install again.
...
| Watchdog start / stop of the tunnel | `service start_wgc N` / `stop_wgc N` | `nvram set vpnc_unit=<row>` + `service restart_vpnc` / `stop_vpnc` (`RouterSlotService.runVpncService`) — the same calls MANAGE makes. Since ID-070 (build 453) the watchdog SCRIPT takes this path too: it resolves the row index out of `vpnc_clientlist` in awk, exactly as `vpncUnitForSlot` does, and falls back to `stop_wgc` / `start_wgc` / `restart_vpnrouting0` only when the slot has no row. The old "the Merlin commands are inert on stock" note is gone rather than settled: it was never proven either way, and with ID-063's handshake gate a stock rebuild that does not truly restart now reports the failure instead of a success. |
...
| Script directory | `jffs2_scripts=1` / `jffs2_on=1` | `mkdir -p /jffs/cfg-pia-wg` (`kRouterAppDir`) |
...
**The same on both firmwares.** A slot's watchdog script is `/jffs/cfg-pia-wg/watchdog_wgcN.sh` (`watchdogScriptPath` in `firmware.dart`). A watchdog counts as ACTIVE when its `cru` entry exists **and** the script is not empty (`[ -s <script> ]`). `fetchSlots` and `getWatchdogStatus` run that same check, and neither reads the NVRAM settings, which survive a DISABLE and a failed deploy alike. `getWatchdogStatus` reports the watchdog enabled only when, as well, `wgcN_enable` is `1` and the interface is up.

**The watchdog relies on `wgcN_enable=1`, on both firmwares, in three places:** the deployed script stands down on an explicit `0` unless it is a deploy run (an empty value does not stand it down); `getWatchdogStatus` requires `1`; and the watchdog screen's SAVE treats a watchdog that is not enabled as new, so it checks the region again. Every enable writes `wgcN_enable=1` before its service call, and every disable writes `0`. Measured on stock 2026-09-15: `1` on all five slots, all enabled, with watchdogs on wgc1 and wgc5 working correctly. **Do not change what these three read, or add writes to `wgcN_enable`, without a hardware test.**
...
**`restart_vpnc` does not stop a tunnel.** The interface stays up in `wg show interfaces`, so stock disable/delete/revert must use `stop_vpnc` (ARCHITECTURE.md "Stop/Disable"). Each of them first writes `wgcN_enable=0` and `vpnc_clientlist` index 5 to `0`, which is what makes the WebUI report "disconnected". Enable writes both to `1` before `restart_vpnc`, and `wgcN_enable` stays `1` afterwards - the watchdog depends on it (see "The watchdog relies on `wgcN_enable=1`" above).
