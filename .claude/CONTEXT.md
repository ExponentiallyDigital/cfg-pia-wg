...
- **The Play Store "what's new" is NOT generated from CHANGELOG.md.** `release.yml` writes one line - a link to the hosted changelog - into `distribution/whatsnew/whatsnew-en-AU`, and that is what each new version code lands with. The field itself takes far more than that, and the real notes are written in Play Console. So a release currently has TWO note-writing steps, one automatic and one by hand, from one source of truth that only feeds the first.
