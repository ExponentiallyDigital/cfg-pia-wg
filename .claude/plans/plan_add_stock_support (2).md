...
Implement firmware detection that triggers exclusively when the user navigates to `manage_router_screen.dart` or `watchdog_management_screen.dart`. Use `nvram get 3rd-party` to check for the word "merlin" If the command fails (non-zero exit code, empty/unexpected output, SSH connection failure, timeout etc), fall back to an error message then return to the previous menu. If successful, set/update an application flag in memory. Once this flag has been set, skip rechecking until the app is exited.
