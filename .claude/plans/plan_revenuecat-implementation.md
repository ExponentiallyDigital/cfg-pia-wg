...
20. [*] **Create the default offering**, one package, containing the product.
    - **Check WHICH product is in the package.** RevenueCat's setup wizard creates the `default` offering with a `$rc_lifetime` package containing the **Test Store** twin, not the Play Store product. It looks complete - offering current, one package, green ticks - and the app cannot sell anything, because Google Play will not price a Test Store product. Attach `lifetime_unlock` to that same package; a package holds one product per app, so both can live in it. **The control is only in the EDIT view:** Product catalog, Offerings, `default`, **Edit**, Packages tab. The package then shows one dropdown per app - Test Store already set, `cfg-pia-wg (Play Store)` reading "No product". The read-only offering page does not expose it, the product page only attaches ENTITLEMENTS, and the offering's three-dot menu offers nothing relevant. Found on hardware 2026-09-12: the paywall rendered perfectly with the buy button disabled and reading "Not available right now".
    - Earlier notes in this plan called the Test Store twin "inert in production". Wrong: it is inert as an ENTITLEMENT, but the wizard puts it in the offering, where it starves the real product. AN: it's already created.
