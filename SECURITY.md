...
- **`/jffs/curllst` (a stock ASUS firmware behaviour, not ours):** the router's own `/usr/sbin/curl` appends the full command line of every invocation to `/jffs/curllst`, which is world-readable (mode 666), survives reboots, and is rotated to `/jffs/curllst.1`. It cannot be disabled. The watchdog's PIA token request passes credentials on the command line, so the deployed script empties the file after every `curl` and on every failure path. Anything else on the router that uses `curl` is not covered by that, so treat the file as sensitive and redact it before sharing.
- **Router SSH credentials** are only ever held in memory for the session and are never written anywhere - not to the device, not to the router. The router *address* is the one exception, and only the address - see "Data handling & privacy" below.
...
- **One value is remembered on the device: the router LAN address.** Since build 410 a successful SSH connect writes the address you connected to into a single-line file in the app private storage, so it prefills next time instead of being retyped every session. It is not a credential - it is a private-range address that identifies nothing outside your own network - and it is the ONLY thing the app keeps. No username, no password, no generated configuration. It is written only after a connect has succeeded, so a wrong address is never stored; `android:allowBackup="false"` keeps it off Google Drive, so it never leaves the handset; and ABOUT -> FORGET ROUTER IP deletes it. A source-scanning test fails the build if that file ever learns to write anything else.
