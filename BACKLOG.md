...
- AN-2026-09-13_001: all LAN devices lost internet while deploying a watchdog to wgc5 with five tunnels up; the outage itself has not been seen again. Cause, traced 2026-09-14. The router's own resolver: dnsmasq sends only the ISP's own domain to the ISP's DNS and everything else to `127.0.1.1`, which is stubby, running DNS-over-TLS to the servers set in the WebUI (WAN DNS Setting) - Quad9 on the test router. The firmware adds `from all to <slot DNS> iif lo lookup <table>` for each tunnel's DNS servers, which on the test router are also Quad9, so stubby's TCP 853 to 9.9.9.9 is routed through the lowest-index tunnel (`ip route get 9.9.9.9` answers `dev wgc5`, and the next tunnel when wgc5 is disabled). Confirmed in conntrack with only wgc1 up: stubby's connections to both Quad9 addresses on port 853 came from the tunnel's own address, while a device pinned to Internet was browsing without a VPN - its lookups still went through wgc1. Every device that resolves through the router - on the default connection or pinned to Internet - therefore depends on that one tunnel carrying TCP 853 to Quad9, which the `us_alabama` server did not. dnsleaktest from a device on the default connection and from one pinned to Internet both reported the same Quad9 resolver and no ISP resolver, so there is no leak to the ISP. The overlap is a coincidence of settings: slot DNS different from the WebUI's DoT servers would leave stubby on the WAN. **Reproduced by simulation 2026-09-14**, after a reboot, with wgc1 the only tunnel: blocking TCP 853 out of it (`iptables -I OUTPUT -o wgc1 -p tcp --dport 853 -j DROP`) made name lookups time out for a device pinned to Internet and for one following the default connection, while ping to 1.1.1.1 kept working for both. A device assigned to wgc1 still resolved: the firmware redirected its query to 9.9.9.9 on port 53 through its own tunnel, and nslookup named the server `UnKnown` because not even the reverse lookup reached the router. Runsheet in `.claude/testing/`.
