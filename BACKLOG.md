...
- ID-066 BUG: **MANAGE DELETE on a slot whose watchdog is paused leaves that watchdog on the router, SMTP password included.** `_deleteManage` calls `stopWatchdog` only when the watchdog is active. `deleteSlot` then unsets the slot's own keys and `wgcN_wd_primary_ip` / `wgcN_wd_secondary_ip`, and nothing else: `wgcN_wd_check_interval`, the email and SMTP keys (`wgcN_wd_smtp_pass` among them) and `/jffs/cfg-pia-wg/watchdog_wgcN.sh` all stay. WATCHDOG DELETE always calls `stopWatchdog`, so only MANAGE is affected. Found by reading the code on 2026-09-15; not yet seen on hardware.

- ID-122 BUG: **the ACTIVE badge is a snapshot, and nothing refreshes it.** `fetchSlots` reads `ip -o link show up` once, when the screen is entered or after an action, and the badge is `activeSlots.contains(slot)` from that read. A tunnel that drops a second later keeps its badge until the next action or the next visit - which is one of the ways a slot can read ACTIVE while nothing is running (ID-092 analysis, 2026-09-19). Wanted: re-read the interface list when the app comes back to the foreground, and on a modest timer while MANAGE or WATCHDOG is open.
- ID-123 BUG: **"up" is not "working", and the badge only knows "up".** The link flag is set as soon as the device exists: a slot whose PIA registration has expired still shows in `ip -o link show up`, sends, and never receives - the state that leaves the router's own web interface stuck on "connecting". ENABLE has required a WireGuard handshake since build 4xx, and the watchdog's rebuild does too as of ID-063; the badge does not. Wanted: badge from `wg show <iface> latest-handshakes` with the same 300-second rule the watchdog uses, so ACTIVE means answered rather than merely present (ID-092 analysis, 2026-09-19).
- ID-124 BUG: **nothing checks that a disable or a revert actually took the interface down.** `disableSlot` and `_revertEnable` write the flags and call the service, then return; the modal's `_refresh` re-reads the link list afterwards, so a slot whose interface survived the call badges ACTIVE straight after the app said it disabled it. That combination is exactly what was measured on stock in the 419 investigation, where `restart_vpnc` cleared the clientlist flag and left the interface up - the reason the disable paths were changed to `stop_vpnc`. Wanted: after a disable or a revert, poll for the interface to go and report it plainly if it does not, rather than leaving the badge to imply it (ID-092 analysis, 2026-09-19).

Not faults, recorded so the next reader does not re-investigate them (ID-092, 2026-09-19):

- A WireGuard SERVER interface is `wgs1`, and `fetchSlots` matches `wgc(\d)` only, so a running server cannot badge a client slot.
- A tunnel someone enables in the router's own web interface badges here too. That is the badge doing its job: it reports the router, not what this app did.
