...
2026-09-24 v0.8.97 build 467 - the release candidate, and a run sheet with only your part in it

- ID-204 TST: **every manual test says where it runs, and the run sheet holds only what you do.** Each title in TESTING.md ends with a tag. `[hand]` (119): you do it and judge it. `[script]` (17): you press the buttons, and `scripts/e2e.sh` on the router does the checking and answers PASS or FAIL - most of DEV, DEF and GRD. `[ci]` (20): an automated test now does it, named in its place, so nobody runs it twice. `[retired]` (2): DEV-9 and DEF-2, which e2e.sh's checks now are. A test that sets up state later tests need stays `[hand]` even when CI covers its checks, so the sequence still works: MAN-2 creates wgc5. Which tests CI covers was worked out by reading each automated test against each manual one; 21 were fully covered, and about 115 more are partly. `dart run tool/runsheet.dart` writes the run sheet: Part 1's groups and their start states, and every `[hand]` and `[script]` test in order, each with a Result line. Its test also checks TESTING.md itself: every test tagged, every label used once, and every `[ci]` test naming a test file that exists.
- ID-208 TST: **TESTING.md rewritten against the notes from the 2026-09-19 run.** Every group states the router it starts from, and that is now true: WD says no watchdog exists and how to check, and raises the stock VPN limit to 4 first, because WD-23 runs four tunnels; DEF moves TABLET back to the default, where DEV leaves it pinned to Internet. Tests carry their own commands rather than "as in BRK-2" or "see R3", and name the slot and the device. Router-side steps in DEV, DEF and GRD are `e2e.sh before`, the button, `e2e.sh after` with the expected rules, guard and exit written out, using shell variables set once per SSH session. Watchdog runs by hand use `foreground`, so the log is complete when it is read. Every per-test note in the item is done, and tests changed by this release's fixes now match them: WD-2 (ID-202), WD-16's colours (ID-158), LOG-7 and SET-3 (ID-155), ABT-1 and ABT-3 (ID-155, ID-157, ID-190), SET-5 (ID-147), EXT-1, and DEF-3 and DEF-8, which now check both tunnels keep running (ID-220). DEF-3 also records whether the router stops the old default when switching between tunnels, which nobody has measured. MAN-15 was used twice: the new one is MAN-17, in the MAN group.
- DOC: README's testing line gives the new counts: 136 manual tests, 17 of them script-checked and 13 needing a Play build, and 20 more now automated.
- TST: 1362 automated tests, 9 new, for the run-sheet extractor and for TESTING.md itself.
- CHG: WIP 4 -> 2 bullets. Moved to this block, done: ID-204, ID-208. ID-210 stays in WIP until the run is done; the run sheet's location is added to it. Nothing removed.
