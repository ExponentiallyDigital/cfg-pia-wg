...
- FIX (next build, needs a decision): **changing a running slot's region on the watchdog form leaves the old server running.** Found 2026-09-14 while fixing the redundant restart: SAVE & DEPLOY writes only the new `wgcN_desc`, and the script's deploy run exits as soon as it sees a handshake under 5 minutes old, so the rebuild onto the new region never happens - the same stale-tunnel-under-a-new-label fault CREATE had. Restarting the tunnel does not help, because it reloads the old server's keys. Options: stop the tunnel and force the deploy run to rebuild it, or refuse a region change on a running slot and send the user to MANAGE.
- FTR (next build): **check the target tunnel before assigning a device to it.** At APPLY on the device assignment screen, and when changing the default connection, check each slot a change moves devices onto: is its interface in `ip -o link show up`, and is its latest handshake under about 3 minutes old (WireGuard re-handshakes every 2 minutes on a live tunnel; watchdog logs show ages up to 111s). Warn rather than block, with a confirm that names the problem: a slot that is not up means the device uses the default connection until the slot is enabled, which is legitimate; a slot that is up with a stale handshake means the device may have no internet. The default connection needs it most, since it moves every unassigned device. A handshake is not proof of a working path - the `us_alabama` server handshook normally while not reaching Quad9 - so this catches "not running" and "server not answering", not every failure. Pairs with the entry on showing where a pinned device is actually exiting.
- FTR (next build): **show where a pinned device is ACTUALLY exiting when its slot is down.** Measured on hardware 2026-09-13: disabling a slot leaves the `vpnc_dev_policy_list` record intact and the router removes the `ip rule`, so the device falls through to the default connection and keeps working. Correct behaviour, but the assignment screen still names the pinned slot, which is true of the configuration and false of the traffic. Proposal: when the pinned slot is not active, mute the slot label and append where it really goes, resolved rather than assumed - the default connection if that is up, otherwise the plain internet, two hops at most. The picker value must NOT change: the pin is intact and re-enabling the slot restores it. The default-connection panel needs the same treatment when the default itself is down.
- TST (next build): **end-to-end hardware test for device assignment**, written into TESTING.md against the behaviour of the two items above, then run as the retest before the release. Device assignment is a signature feature and has to be demonstrably solid, not just fixed. For each case, confirm on the router (`ip rule show`, `nvram get vpnc_dev_policy_list`, `wg show interfaces`), on the device (exit location, name resolution) and in the app (assignment screen, exit shown):
  - **moving a device between tunnels:** only the new rule exists, the old one is gone, the device exits the new region.
  - **moving a device to and from the default connection:** the per-device rule is removed and restored, and the device follows the default.
  - **assigning to a disabled slot:** the device uses the default connection until the slot is enabled, then moves to it without being reassigned.
  - **deleting and recreating a slot with devices assigned:** the devices are released on delete and the recreated slot carries no stale pin.
  - **changing the default connection:** every unassigned device follows it and no assigned device does.
  - **a reboot:** every assignment and the default connection survive and are applied before the first watchdog run.
- FTR (decision first, then build): **a pinned device's DNS has no fallback.** Stock redirects a pinned device's port 53 queries, UDP only, to the slot's FIRST DNS server (`VPN_FUSION` DNAT); the second is never used, so one unreachable resolver means no DNS at all for that device. Consider documenting it by the DNS field, or defaulting to resolvers the tunnel can always reach.
