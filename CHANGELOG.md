...
**Drafted 2026-09-17, from the DNS analysis: five items sequenced ** (ID-080 and ID-075 have been moved from this list and will be implemented on 2026-09-18). The order matters: ID-075 fixes the sentence the rest quote; ID-076 is the change that makes the watchdog's own lookups private and everything after it easier; ID-077 finishes what ID-076 cannot; ID-078 is the missing half of ID-006; ID-079 and ID-080 are what the user reads; ID-081 is a decision, not code. What prompted them: Andrew read that the watchdog resolves names in the clear, having assumed the router encrypted everything when DoT/DoH are set in the router's DNS and all devices are assigned to slots.

- ID-076 SEC: **the watchdog resolves names over encrypted DNS, so nobody between the router and its DNS server learns which addresses it looks up.** Today the deployed script resolves `raw.githubusercontent.com` (the PIA CA, when not cached), `www.privateinternetaccess.com` (the token) and `serverlist.piaservers.net` (the server list) with ordinary DNS. `addKey` already resolves nothing, because the script passes the chosen address with `--resolve`. Stock's curl can do this today: `curl 7.84.0 (arm-buildroot-linux-gnueabi) libcurl/7.84.0 OpenSSL/1.1.1t`, with `--doh-url` in `curl --help all`, measured on the router 2026-09-17. Add `--doh-url` to the script's `CURLB`. Three constraints: point it at an **IP literal** whose certificate covers that address, so there is no name to resolve first; **choose an address no slot uses**, because the firmware's `iif lo` rules would otherwise route the watchdog's own lookups into a tunnel, which is the failure it exists to repair (ID-001, ID-075); and **fall back to plain resolution** if the flag fails, since a watchdog that cannot rebuild a tunnel is worse than one whose lookups are visible. Decide whether the DoH address is a constant or a field on the watchdog form. Keep `addKey`'s `--resolve` and the cached CA as they are: both already avoid lookups. Note for the writing: what an observer learns today is that the router talks to PIA, which the WireGuard traffic already shows; the SMTP hostname in ID-077 is the revealing one.
- ID-077 SEC: **decide what to do about the alert email's hostname lookup, which ID-076 cannot cover.** `mailsend-go` on stock and `openssl s_client` on Merlin both take the SMTP hostname and need it for certificate verification, so the lookup happens inside them, in the clear. It names the user's email provider, which nothing else on the wire reveals. Three ways: accept it and say so in the README; resolve it first through ID-076's resolver and write a temporary `/etc/hosts` entry so the mailer resolves locally with the hostname intact, remembering that stock's `/etc/hosts` is firmware-generated and the entry must be removed on every exit path; or, Merlin only, `-connect <ip>:<port> -servername <host> -verify_hostname <host>`. The failure diagnostic's `nslookup` is deliberately the system resolver and should stay as it is. Do this after ID-076, and reuse whatever it builds.
- ID-078 FIX: **a tunnel that handshakes but resolves nothing should be rebuilt, not passed.** The missing half of CHANGELOG ID-006, which recorded devices on `wgc4` losing name resolution for two days while the watchdog logged a healthy handshake every five minutes. After the handshake check passes, probe the slot's own first DNS server through its tunnel (`nslookup <name> <slot DNS>`), and treat a failure as a dead tunnel. Two constraints: only the **highest-numbered** slot using an address owns the `iif lo` rule for it (ID-075), so the probe tests the wrong tunnel when addresses are shared - skip it then, or add a temporary `ip rule ... lookup <table>` for the probe address and remove it on every exit path; and require **two consecutive failures**, so one lost packet does not bounce a healthy tunnel. `ping -I` and `curl --interface` do not work here, which ID-006 records. Related: ID-063, because "Reconfig SUCCESS" today means only that the interface came up, and ID-070, which asks whether the rebuild's `start_wgc` does anything at all on stock.
- ID-079 DOC: **publish the DNS explainer, with its three corrections.** Andrew has written a full section on where lookups actually go: the router answers for devices that are not pinned, so the default connection never sees them; a pinned device's lookups follow its own slot; and a slot sharing an address with the router's DoT list can take DNS from the whole LAN. Checked against the code 2026-09-17 and correct, except: it states that the app **warns** when a slot's DNS matches the router's DoT Server List, which is CHANGELOG ID-005 and is not built - nothing in `lib/` reads `dnspriv_rulelist`, so the text promises a safety net that does not exist; it names Andrew's router model, which CONTEXT forbids; and its "For the technically inquisitive" diagram crosses its own edges, which splitting it in two fixes (who answers a lookup, then where that lookup goes). Two claims are his own measurements rather than anything the repo records, and should say so: that the firmware appends `nameserver 127.0.1.1` last when DoT is on, and PIA's in-tunnel resolver addresses. Decide where it lands: README section, or its own document linked from it. Wording waits on ID-011, as ID-005 does. Related: ID-020, ID-068.
- ID-104 DOC: add to README that it's recommended to pin devices to a specific slot rather than follow a default connection. This helps steer DNS traffic flows through the tunnel following device traffic, rather than over the default connection. That can occur if both your router and tunnels use the same DNS addresses. See <insert a PLACEHOLDER to the yet to be created section in README that describes this>.
- ID-081 INF: **decide whether the app's own lookups on the phone matter.** `lib/pia_service.dart` resolves the same three PIA names through Android's resolver, so STANDALONE and CREATE are in the clear unless the user has Private DNS switched on. ID-076 does not change this: it covers the router only. Either document it as a prerequisite alongside the rest of the DNS advice (ID-079), or resolve those three names in Dart over DoH, which is more code than the exposure justifies on today's evidence. A decision, not necessarily a change.
- ID-005 ADD: **say when a slot's DNS is the same as the router's own encrypted-DNS servers.** When they match, the firmware's per-tunnel `iif lo` rule sends the router's own DNS-over-TLS - the lookups for every device that resolves through the router - through the first tunnel, so that one tunnel failing takes names away from the whole LAN (BACKLOG ID-001: the router's own DNS follows the first tunnel). The app must not change either setting (CONTEXT: the user's WebUI DNS settings are theirs), so this is information only: a note at stock CREATE and EDIT naming the overlap. Measured 2026-09-14: the WebUI's list is `dnspriv_rulelist`, as `<IP>port>hostname>` records (`<9.9.9.9>853>dns.quad9.net><149.112.112.112>853>dns.quad9.net>` on the test router), with `dnspriv_enable` `1` while DNS-over-TLS is on. Waits on ID-011, Andrew's router test in BACKLOG: its result decides this note's wording and the README prerequisite. Two corrections from the 2026-09-17 DNS analysis: "the first tunnel" above should read the highest-numbered slot sharing the address, per BACKLOG ID-075; and BACKLOG ID-079 records that Andrew's DNS explainer already tells users this warning exists, so the text and the code disagree until this is built.
- ID-006 FIX: **the watchdog passes a tunnel that handshakes but cannot resolve names.** Measured on hardware 2026-09-13/14: devices pinned to `wgc4` got no answer from 9.9.9.9 on UDP 53 or TCP 853 (conntrack `[UNREPLIED]`), while ping to 9.9.9.9 and HTTPS and DNS over HTTPS to 1.1.1.1 worked through the same tunnel. The server was `us_alabama`. It seemed to recur after `wgc4` was recreated on `aus_perth`, but the WireGuard peer key and tunnel address were unchanged - the recreate never reached the running tunnel (see the CREATE entry above) - so both runs were Alabama. The same Quad9 redirect worked on `wgc3` (`france`), and every slot's firewall rules, allowed IPs, DNS and MTU were identical, so nothing points at the slot. Devices pinned to it had no name resolution, and the watchdog logged a recent handshake every five minutes and never moved it to another server. Confirmed 2026-09-14: after `wgc4` was deleted and recreated on `aus_perth` (new peer key, exit shown as Perth), a device pinned to it resolved names through the same Quad9 redirect. Needs a probe that exercises the slot's own DNS servers through the tunnel. The design is open: on stock the router's own traffic is not routed into `wgcN` (`to <dns> iif lo lookup <table>` matches the lowest table first), which is why `ping -I` was abandoned. Design input, 2026-09-14: the router's OWN lookup (`nslookup <name> 127.0.1.1`, through stubby) travels through the FIRST tunnel whenever slot DNS matches the router's DNS-over-TLS servers, and failing it takes name lookups from every device that resolves through the router (BACKLOG ID-001, reproduced). It is cheap to run on the router and worth probing - but it tests that first tunnel, not the watchdog's own slot, so it is one half of the design, not all of it. Sequenced last: designed with Andrew once ID-011's router test is done. The other half is drafted as BACKLOG ID-078: probe the slot's OWN first DNS server through its tunnel, skip or add a temporary rule when a higher-numbered slot owns that address (BACKLOG ID-075), and require two consecutive failures.
- ID-001 FTR: **the router's own DNS follows the first tunnel, not the default connection** - firmware behaviour, recorded. All LAN devices lost internet while deploying a watchdog to wgc5 with five tunnels up, an outage not seen again. Traced 2026-09-14 on stock: a LAN device that asks the router - unassigned, or pinned to Internet - is answered by dnsmasq, which sends everything but the ISP's own domain to stubby at `127.0.1.1`, which uses DNS-over-TLS to the servers in the WebUI's WAN DNS Setting. The firmware adds `from all to <slot DNS> iif lo lookup <table>` for each running tunnel's DNS servers, so when those match the WebUI's servers (Quad9 on the test router) stubby's port 853 leaves through the lowest-numbered table's tunnel, whatever the default connection is - traffic the router creates never follows the default, whose rules match `iif br0` and `br1` only. **Reproduced** after a reboot with wgc1 the only tunnel: blocking TCP 853 out of wgc1 timed out lookups for a device pinned to Internet and for one following the default, while ping kept working for both; a device assigned to wgc1 still resolved, because the firmware sends its DNS to the slot's first server on port 53 through its own tunnel. The router's own programs - the watchdog's `mailsend-go` and `curl`, `nslookup` - are not on this path: `/etc/resolv.conf` lists the WAN's DNS first, and a test email went out while blocked, so the 2026-09-06 undelivered alert (`server misbehaving`) is unexplained again. The app must not change the user's DNS settings (CONTEXT), so its only response is information - see CHANGELOG ID-005. Runsheet in `.claude/testing/`. **The title is wrong and ID-075 corrects it:** the rules match the lowest table first, which is the HIGHEST-numbered slot, not the first tunnel - measured September 2026 as `wgc1` 9 through `wgc5` 5. This item was reproduced with only `wgc1` up, which could not tell the two apart. See also ID-076 (the watchdog's own lookups must not depend on any of this) and ID-080.
...
- ID-110 GUI: **links lose their underline but stay tappable:** the header's "Exponentially Digital" and version number, and ABOUT's link row.
- ID-109 GUI: **HOME's two footer lines line up with the menu.** Indent "how to use this app" and "add a Play Store app review" so their icons sit in the same column as the nine menu icons, remove the underline, and colour both icons #BFB27C (muted khaki-gold). They stay tappable.
...
- ID-090 GUI: **DEVICE ASSIGNMENT's two buttons sit side by side before anything is staged.** With nothing selected they stack vertically and only line up once a change is staged: the label reads APPLY 0 CHANGES, wide enough to wrap the row.
...
- ID-106 DOC: **README, DEVICE ASSIGNMENT: a note that the device list comes from the router's firmware** and can lag - a device may read offline until it sends traffic, and a "ghost" entry can linger, particularly with a random MAC. Links to ID-105's section.
...
- ID-119 DOC: **CONTEXT tells Claude to write Markdown prose to the voice guide** at `.claude/testing/voice-guide.md`, CONTEXT itself excepted, and a test fails with a clear explanation if that file is ever tracked by git.
- ID-091 CHG: **RESTORE PURCHASE says what actually happened.** It reports "Purchase restored. Everything is unlocked." even when the app was already licensed. Wanted: an already-licensed message; a restored message only when the state changes; and on failure a plain-English popup with the detail in the app log.
- ID-097 GUI: **no missing-binaries error after a successful install.** On a fresh install following UNINSTALL, licensed, entering WATCHDOG offered `jq` and `mailsend-go`, both installed, and the app log still carried the red "Unable to locate" notice naming `/jffs/cfg-pia-wg/jq`.
- ID-098 FIX: **uninstall puts the maximum active VPNs back to 2** if the app raised it.
- ID-084 FIX: **an SSH reconnect is not always written to the app log.** Seen 2026-09-16 after activity on ABOUT: the router log recorded a fresh password auth with no matching "Router SSH connection dropped; reconnecting." line in the app log, while a later reconnect that day did log one.
- ID-092 FIX: **MANAGE can show ACTIVE for a slot that is not up.** Analyse every path that can produce it - the badge comes from `wg show interfaces` when the list is read, so a stale read, a tunnel dropping after the read, and an enable that failed with the flag left set are all candidates - and write the findings into BACKLOG section 2 as new items. No code changes unless they overlap work already scheduled for this build. Two earlier fixes are behind it: `activeSlots` replacing a single-slot badge, and a badge that stayed after DISABLE.
- ID-094 GUI: **offer RECREATE when an enable fails because the server never answered.** Add a house-colour button to that popup which runs the CREATE flow for the slot - PIA credentials dialog, DNS choice, then the usual build - for a configuration whose PIA registration has gone stale.
- ID-095 FIX: **DISABLE pauses a watchdog, DELETE removes it, and both say so.** MANAGE DISABLE currently tears the whole watchdog down - schedule, script and settings - while its dialog says only "stops its watchdog". Agreed 2026-09-18: DISABLE takes the slot down and pauses its watchdog, keeping the settings and script so ENABLE restores it; DELETE removes the watchdog and the slot together, after a popup that says so and is accepted. Settles the other half of ID-066.
- ID-099 GUI: **the router log loses your place when older pages load quickly.** Scrolling back many screens in quick succession, fetching the next segment jumped the view down by several screens: it neither returned to the newest lines nor held position.
- ID-111 GUI: **new launcher icon and a native splash screen.** `flutter_launcher_icons` takes `image_path: assets/icon/app_icon_legacy.png`, `adaptive_icon_background: "#001620"` and `adaptive_icon_foreground: assets/icon/app_icon_foreground.png`, keeping its other settings. Add `flutter_native_splash` as a dev dependency with `color: "#001620"`, `image: assets/icon/splash_detail.png`, android and ios true, and an android_12 block using the foreground image with `icon_background_color: "#001620"`. Declare `assets/icon` under flutter assets if it is not already, regenerate with `dart run flutter_launcher_icons` and `dart run flutter_native_splash:create`, and update `build.ps1` and `build.sh` to run both. The three new assets are tracked as of 2026-09-18 and the old icon files are removed; nothing but pubspec and the generated platform files changes.
