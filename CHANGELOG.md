...
- ID-165 FIX: in DEVICE MANAGEMENT, a device is showing up as online when it has been switched off for > 48 hours. Check the code path that reads the ASUS list of devices; in the ASUS WebApp, that device is correctly showing as offline. More evidence from DEV-13 on 2026-09-21: TABLET was switched off at 09:34 and the WebUI showed it offline at once, while the app still showed it online until 09:40, surviving an app restart. At 09:39 the app also showed the NAS offline while it was online. Find which router file the app reads presence from, and how stale it can be compared with what the WebUI uses. **2026-09-24:** the app reads presence from `/jffs/nmp_cl_json.js` alone; which source the WebUI uses has never been measured, so no fix is guessed. `scripts/presence-probe.sh <MAC>` prints every candidate side by side - both device files, `/tmp/clientlist.json`, `ip neigh`, the Wi-Fi association lists and the DHCP lease - every 30 seconds. Run it in the release-candidate run (ID-210) while a device is switched off and on, and read the answer off it.
...
2026-09-24 v0.8.96 build 466 - README says what the kill switch is, and what it is not

- DOC: README says plainly that the app gives stock firmware a kill switch of its own, built from the router's routing rules, in Features and in DEVICE ASSIGNMENT, with what it does not cover: devices that follow the default rather than being pinned, the few addresses the router always sends straight out, and IPv6, untested. Also fixed there: changing a watchdog's region no longer claims pinned devices use the default connection meanwhile; since build 460 they have no internet until the tunnel is back.
- ID-145 DOC: README 5.3.1 notes that Gmail files a message sent from and to the same address under Sent, not the inbox, so an alert to yourself looks lost. Well-known Gmail behaviour, not tested here: worth one look in the release-candidate run.
- ID-146 DOC: README Features: the watchdog looks up PIA's servers and your mail provider over encrypted DNS, so whoever can see the router's DNS cannot see who you use, and it falls back to a plain lookup once, logged, rather than leave a tunnel down.
- ID-148 DOC: README Features, "Type it once": one router login serves every screen for the session, and PIA and email settings from one watchdog are offered for the next.
- ID-149 DOC: README Features: the log colours, the same on ROUTER LOG and the watchdog log.
- ID-162 DOC: README 4.1.1: Download Master may drop off the router's USB Application page once the app has taken over its start-up script. Nothing is broken, and UNINSTALL puts the original back.
- ID-177 DOC: README's build-chain list says how much testing is behind a release: as at build 466, 1353 automated tests, an 80% line-coverage target with the live figure on the Coverage badge, and 158 manual end-to-end tests, 13 of them needing a Play build, in a run sheet anyone can read.
- ID-197 DOC: ARCHITECTURE 7.3's rebuild diagram, whose last line was clipped, is redrawn with short label lines, and brought up to date while there: one restart per firmware rather than Merlin's three calls on both, the wait for the router's service queue, the new-key check and its one retry (ID-214), and the 20-second handshake gate. The paragraph under it says the same. The other three diagrams were read, not rendered: nothing in them looked like the clipped case, but a look at them on GitHub would settle it.
- CHG: WIP 11 -> 4 bullets. Moved to this block, done: ID-145, ID-146, ID-148, ID-149, ID-162, ID-177, ID-197. Nothing removed.

2026-09-24 v0.8.95 build 465 - a promote you can read, on a runtime that is not retired

- ID-142 REL: the release run in the Actions list is named after the tagged commit, which starts "vX build N", so it says which build was pushed: "Release v0.8.95 - v0.8.95 build 465 - ...". A tag on a merge commit shows GitHub's merge message first, with the PR title after it.
- ID-178 REL: **promote asks for the build number and puts it first in the run's name**: "Promote build 465 - internal to alpha". It now defaults to internal to alpha, one step at a time; the targets are alpha, beta, then "** PRODUCTION **", last and marked. A new DRY RUN option shows what would move and changes nothing.
- ID-179 REL: **promote no longer uses an action on the retired Node 20.** kevin-david/promote-play-release still declares Node 20 and has had no release since April 2025, so there is no update to take. `scripts/play_promote.py` does what it did, with the same defaults, in about 60 lines: open an edit, copy the source track's releases to the target at full rollout, commit. Its two libraries are pinned. Not yet run against Play: use DRY RUN for the first promote, which reads the tracks and throws the edit away.
- ID-176 REL: CLOSED - working as intended. CodeQL can build an overlay database only with `build-mode: none`, and for java-kotlin that mode does not analyse Kotlin. `MainActivity.kt` is the app's Kotlin, 237 lines including the clipboard channel, so `manual` stays and the notice - "falling back to creating a normal full database" - is the price of scanning it. Original ask: fix warning in `Quality & security` actions script: "CodeQL (java-kotlin) Cannot build an overlay database because build-mode is set to "manual" instead of "none". Falling back to creating a normal full database instead."
- CHG: WIP 15 -> 11 bullets. Moved to this block: ID-142, ID-178 and ID-179 done, ID-176 closed. Nothing removed.

2026-09-24 v0.8.94 build 464 - fourteen fixes you will notice, and a login that is kept

- ID-144 FIX: **the watchdog's DNS note names the real clash.** It said the address was "also used for DNS on this router" whenever it matched either this tunnel's own DNS or the router's encrypted DNS. The common case is neither the router nor another tunnel: it is this tunnel's own DNS server, which the router reaches through this tunnel, so a broken tunnel takes the watchdog's lookups with it. The two causes now have their own words. The note also follows the fields as you type; it used to change only on some unrelated rebuild.
- ID-182 GUI: **the DNS note says what is true.** "Devices assigned to this VPN send their lookups to the first server only. The router itself can use both." Measured 2026-09-24: the firmware redirects a pinned device's lookups to the first address, while its own rules cover both for the router. README and TESTING say the same.
- ID-170 GUI: choosing "Something else" for the watchdog's own encrypted DNS now empties both fields, rather than leaving the last resolver's URL and address there.
- ID-147 CHG: the stock VPN limit is read when it is checked, at ENABLE and at CREATE/EDIT, not when the slot list was loaded. Raised in SETTINGS, it stayed at its old value until the screen was left and entered again.
- ID-155 FIX: **a login from ABOUT, SETTINGS or ROUTER LOG is checked before its dialog closes, and kept for the session.** The dialog closed first and the login was tried afterwards, so a password manager was never offered the credentials: closing the dialog had cancelled that. Now CONTINUE logs in, and only once the router has accepted it does the dialog close, offer to save the login, and mark the session connected, so MANAGE and the rest reuse it. A login that fails keeps the dialog open with the reason. CANCEL at ROUTER LOG's first login goes back to where you came from, or HOME, not to an empty log.
- ID-201 FIX: MANAGE remembers the router address, and offers to save the login, as soon as the login is proven. It waited for the firmware and helper checks, so NOT NOW at the helper install prompt stored neither, which is what CON-3 ran into on 2026-09-19.
- ID-202 FIX: the watchdog form reports every problem at once, in the order the fields appear, the region first. A made-up region was reported only once every field below it was right.
- ID-156 FIX: the drawer marks HOME when you are on HOME, with the same fill as every other entry, and that fill is lighter so it reads as one. HOME had no fill at all, and the others' was four shades off the drawer.
- ID-157 CHG: when the router's watchdog script is newer than the app - a Play build installed over a newer sideloaded one - ABOUT shows the app's own version in red, the script's in amber, and keeps UPDATE WATCHDOG VERSION.
- ID-158 FIX: **the watchdog log is coloured like ROUTER LOG.** Lavender for the watchdog, red for a fault by ROUTER LOG's own rule, so a line is red on both screens or on neither, and teal for a rebuild that worked. It had a scheme of its own: amber for the steps of a rebuild, which everywhere else here means a warning, and red for "Not connected yet", which is what a first deploy says before its tunnel is up.
- ID-168 GUI: VIEW ROUTER WATCHDOG LOG takes ROUTER LOG's colour. It was grey, and read as a disabled button.
- ID-190 GUI: updating the watchdog script writes the same sentence to both logs, version included: "Watchdog script updated to v0.8.94 build 464 for wgc5". The app log said only the path and its size.
- ID-189 TST: `lib/entitlement.dart` from 20% to 50% line coverage: every store error the paywall can meet reads as a sentence with its code, a pending payment is told apart, and a build with no store key sells nothing and never reaches the SDK. The rest calls the store SDK itself. ABOUT (85.2%) and `app_shell.dart` (81.6%) are above the 80% bar and were left. Overall 95.0%.
- ID-217 REL: the three javac "value 8 is obsolete" warnings are silenced for plugin modules, with the switch javac itself names (`-Xlint:-options`). The app compiles at Java 17 and keeps every warning. A debug build printed none; to be confirmed on the next `build.ps1` run.
- TST: 1353 automated tests, 20 new.
- CHG: WIP 29 -> 15 bullets. Moved to this block, done: ID-144, ID-182, ID-170, ID-147, ID-155, ID-201, ID-202, ID-156, ID-157, ID-158, ID-168, ID-190, ID-189, ID-217. Nothing removed.

2026-09-24 v0.8.93 build 463 - the screen keeps up, SAVE applies, and DELETE stays deleted

- ID-174 FIX: **a failed ENABLE that cannot stop its tunnel says so.** The revert's own stop result was thrown away, so the user was told the slot was left disabled while its tunnel was still up. The message now adds that the tunnel is still up on the router, as DISABLE already does (ID-124).
- ID-218 FIX: **DEVICE ASSIGNMENT keeps up with which tunnels are running.** It read them once, at connect, so after a reboot every device read "not running - no internet" while every tunnel was up, and going to MANAGE and back changed nothing. Now it checks again the moment the screen comes back into view, and every 15 seconds while it stays there: one round trip, which writes nothing to the app log. Pull down on the list to read the devices and the tunnels again. Staged changes are kept either way.
- ID-203 CHG: **SAVE on a running slot applies the new settings at once.** A running tunnel kept its old settings until it was next started, which MAN-11 found on 2026-09-19. SAVE now asks first - "Saving restarts wgc1. Anything using it drops for a few seconds." - then saves and restarts it, and checks it as ENABLE does: the interface back, talking to the peer now saved, and a handshake since the restart. A restart the router throws away is sent once more (ID-214). If the server does not answer, the settings stay saved and the message says pinned devices have no internet until it does. CANCEL keeps the editor open with your edits in it. New test MAN-15.
- ID-209 FIX: **DELETE no longer restarts the tunnel it is deleting.** Reviewing DEVICE ASSIGNMENT end to end, with the ID-206 model now covering DELETE, found one fault: deleting the slot that was the default connection ended with `restart_vpnc`, and `vpnc_unit` still named that slot's row. The tunnel DELETE had just stopped came back up, running with no profile. The same fault as ID-172, in a path that fix did not reach. Also through `RouterServiceQueue` now: DELETE's `restart_default_wan` and `restart_vpnc_dev_policy`, the last service calls that were not. The rest of the review held: the sweep names priority 100 (since 460), and DISABLE and DELETE run the guard in the right order.
- TST: 1333 automated tests, 12 new.
- CHG: WIP 33 -> 29 bullets. Moved to this block, done: ID-218, ID-174, ID-203, ID-209. ID-165 stays in WIP until the release-candidate run has measured it; the next step is added to it. Nothing removed.

2026-09-24 v0.8.92 build 462 - the watchdog run for real in CI, and a model router that found a bug

- ID-173 GUI: CLOSED - every known way to reach the split state is fixed (ID-172, build 458) or will be reported when it happens (ID-174, ID-214, and DISABLE's own check, ID-124), so a badge would only be a net for unknown causes. Decided with Andrew 2026-09-24. Original ask: MANAGE badges a slot ACTIVE whenever its interface is up and handshaking, and never looks at whether VPN Fusion has the profile enabled. The two can disagree - ID-172 is one way to get there, and the row then reads `ACTIVE` while ENABLE is also offered, which is the contradiction that gave the bug away. Wanted: keep ACTIVE meaning up and answering, and add a warning when the interface is up while...
- ID-163 DOC: CLOSED - answered by measurement on 2026-09-24: the firmware's VPN_FUSION redirect names one address, the first. The on-screen wording is ID-182, and README gets one line in the docs batch. Original ask: why do devices use only the first DNS server when two are specified, eg in MANAGE > EDIT
- ID-169 BUG: CLOSED - split into ID-192, which measured the path on 2026-09-24 and found it reliable (30 of 30 lookups through each tunnel), and ID-193, the race between two watchdogs that also removes the doubled "wgc1: wgc1:" prefix. Original ask: is the DNS probe working correctly? Received "Sep 19 09:30:06 cfg-pia-wg: wgc1: wgc1: no answer from 9.9.9.9; one more and it counts as broken" (FIX the repeating log text "wgc1: wgc1:"). Do we need additional logging from the DNS probe in the router log? Context from the router log:
- ID-151 TST: CLOSED - the TESTING.md rewrite (ID-208) and the router check script (ID-207) write the STARTED and ENDED markers. Original ask: add `logger **TEST XXX-n STARTED**` to all manual tests in TESTING.md so we can easily reconcile what was tested with what the router log contains. Also add `logger **TEST XXX-n ENDED**`. See `.claude\testing\2026-09-19_e2e.md` for examples of how I used this, it made  it much easier to see what was happening.
- ID-200 BUG: CLOSED - the rewritten DEF-3 reads `vpnc_default_wan` only after the app reports the change applied; if the key really fails to change, the release-candidate run (ID-210) shows it. Original ask: DEF-3 on run 1 (2026-09-21) read `vpnc_default_wan` as `9` after the default was switched from wgc1 to wgc5, whose value is `5`, and was marked PASS on TABLET's exit IP. Either the read came before the apply finished - the change takes about six seconds - or the key was not written. Recheck on the rerun: read the key only after APP LOG shows "Device assignments applied.", and if it still reads...
- ID-167 FIX: CLOSED - working as designed: UPDATE WATCHDOG VERSION replaces the script file and nothing else; the deploy email comes from SAVE & DEPLOY, or from the watchdog's next rebuild. Original ask: after a failed deploy due to the script being unable to land on the router, and followed by manually updating the script via the ABOUT menu, no deployment email was sent/received (test email was successfully received).
- ID-205 TST: **the watchdog script now runs for real in CI.** `test/watchdog_harness.dart` runs the generated script under a POSIX shell against a fake stock router: stand-ins for `nvram`, `ip`, `wg`, `service`, `ping`, `nslookup`, `curl`, `jq`, `logger`, `cru` and `sleep`, with state kept in files. 28 tests in `test/unit/watchdog_behaviour_test.dart` cover a healthy check, every rung of the backoff ladder, the WAN outage, the two-strike DNS rule, a rebuild to SUCCESS through VPN Fusion on the right `vpnc_unit`, an interface that is down (BRK-3), one that never handshakes, a rejected PIA login (BRK-5), a slot with no DNS (BRK-9), skipped restarts and two watchdogs at once. Three of them failed on the script as it was, which is the point: ID-192 was a logic fault that passed every text check and was found by hand. BRK-1 and BRK-2 stay on hardware as the check that a real PIA server agrees.
- ID-206 TST: **a stock router that behaves like one, for DEVICE ASSIGNMENT.** `test/stock_router_model.dart` keeps NVRAM, `ip rule` and the running tunnels, and changes them the way ARCHITECTURE 6.8 measured: `restart_vpnc_dev_policy` adds a rule per record every time and never removes one, `stop_vpnc` and `restart_vpnc` act on whatever `vpnc_unit` names, `restart_default_wan` resets the key and stops the default's tunnel, a busy router drops service calls, and a reboot clears the rules. The app's own services drive it, and after every step the test asks the question the user asks: where does each device's traffic leave? A pinned device leaves through its tunnel or not at all, and a tunnel that is switched on is running. 55 tests: the DEV and DEF steps, fail closed through DISABLE, expiry, rebuild and reboot, and 40 seeded random sequences of 25 steps each. Does it catch real faults? Taking out the ID-220 fix fails 24 of them, and taking out the ID-183 sweep fails 30. It raised one more on its first run: see ID-220. It knows only what has been measured, so hardware keeps a short exit-IP set.
- ID-207 TST: **one word per hardware step.** `scripts/e2e.sh <test-id> before` writes the STARTED marker to the router log and takes a snapshot; `after` takes another, prints only what changed, writes ENDED with the result, and checks any expectations passed in: `exit <ip> wgc5` (the kernel's own routing decision for that device, via `ip route get ... iif br0`, so nothing has to run on the device), `rule`, `norule`, `guard`, `noguard`, `default`, `up` and `down`. It answers PASS or FAIL. `scripts/exit-ip.ps1 -Expect NZ` is the device's half: where its traffic really came out, or BLOCKED. Changes nothing on the router but the two log lines. 5 tests run it against stand-ins.
- ID-193 FIX: **two watchdogs no longer trip over each other's DNS probe.** The probe now takes a lock, `/tmp/cfg-pia-wg-dnsprobe.lock`, so only one runs at a time across every slot; a lock more than 20 seconds old was left by a killed run and is broken. And each run's clean-up sweep now removes only its own slot's leftover rule. It used to remove every slot's, including the one another watchdog was using that second. That was the second way wgc5 got charged for wgc1's dead tunnel. The doubled "wgc1: wgc1:" prefix is gone too.
- ID-192 FIX: **two DNS misses in a row now rebuild the tunnel, as they always should have.** The first miss wrote its strike, and the success block in the same run deleted it, so the count never reached two. The strike file is now cleared only by an answer or by a rebuild that worked. Safe to turn on now: the path measured clean (30 of 30 through each tunnel), and ID-193 removed the race that charged a healthy slot for another's misses.
- ID-199 FIX: **the backoff ladder was never broken; the test was.** `scripts/test-backoff.sh` ran the watchdog with no argument, which is how cron runs it, so the script handed itself to the background and returned at once, and the test read the log before anything was in it. The watchdog has a new mode, `foreground`, a cron check that stays attached, for tests and for running it by hand over SSH. `test-backoff.sh` and the manual loop in TESTING.md use it. The harness walks every rung in CI: 120, 240, 480, 960, 1800, 3600 and 5400 seconds, and 5400 again at 12, with no PIA traffic.
- ID-214 FIX: **a restart the router throws away is noticed, and sent again.** The router runs one service at a time, and a call made while another is marked busy waits 15 seconds and is then dropped without a word. Before the watchdog restarts a tunnel it now waits for the router to be free, for up to a minute, and clears a marker that has sat unchanged for ten seconds with its process gone. After the restart it checks the tunnel is talking to the new server's key. If not, the router skipped it: wait, and send it once more; if that is skipped too, the alert says "the router skipped the restart", not that PIA failed. This also closes a false SUCCESS: a skipped restart during a real expiry left the old peer with an old handshake, which passed every other check. In the app, APPLY and the default-connection change now make their service calls the way MANAGE does, through `RouterServiceQueue`: a ghost marker cleared first, and each call finished before the next.
- ID-220 FIX: **changing the default connection leaves your tunnels running.** Switching from a tunnel to Internet stopped the outgoing tunnel and nothing started it again, so it stayed down until its watchdog rebuilt it: a PIA token and an alert email for an outage the app caused. Without a watchdog, it stayed down. The app now restarts the outgoing tunnel at the end if it is still switched on, and a switched-off one stays off (ID-172). The ID-206 model raised a second case on its first run: switching from one tunnel to another, `restart_default_wan` stops the outgoing default, and nothing restarted it. Whether the real router does that has never been measured, so the app now checks, and restarts the outgoing tunnel only if it is down. DEF-8 in the release-candidate run (ID-210) confirms both on a real router.
- TST: 1321 automated tests, 90 of them new: 28 for the watchdog script, 55 for the router model, 5 for e2e.sh and 2 for APPLY. The watchdog script stays under its 38912-byte tripwire: six of its longest comments were cut to a line or two, since they repeated what ARCHITECTURE already says.
- CHG: WIP 41 -> 33 bullets. Moved to this block, done: ID-205, ID-206, ID-207, ID-193, ID-192, ID-199, ID-214, ID-220. Nothing removed.
