...
- CONFIRM: AGREED: the mesh node and the router itself are never offered as assignable devices.
- CONFIRM: a device behind the AiMesh node assigns like any other. Its sources check out (`nmp_cache.js` has the IP, ARP has the MAC); only the end-to-end assignment is still unproven.
