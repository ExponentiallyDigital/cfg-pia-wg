...
- TEST: run `scripts\verify-device-assignment.sh` - answers Q9 (is `restart_net_and_phy` needed, or will `restart_dnsmasq` do) and Q10 (does a per-device assignment fail closed).
- DECIDE: Q9 result decides whether the assignment screen needs a "this interrupts the whole network" warning.
- DECIDE: Q10 result decides whether the feature can be described as protection at all - in the app, README and Play listing.
- DECIDE: should guest-network devices (`br1`) be offered as assignable at all? They cannot reach the LAN.
- TEST: write the device-assignment runsheet before implementing, per the usual pattern.
...
- CONFIRM: reading index 0 rather than presence - a pristine router seeds disabled placeholder records for every reserved device, so "in the list" is not "assigned".
- CONFIRM: the app preserves policy records it did not create, rather than writing only its own.
- CONFIRM: reserved vs not comes from `dhcp_staticlist` membership, never the WebUI `IP Method` column - that column reclassified itself overnight with no config change.
- CONFIRM: the mesh node and the router itself are never offered as assignable devices.
- CONFIRM: a device behind the AiMesh node assigns like any other. **Now known to be seen differently by the main router** - it reports them as `Static IP` with no reservation - so this needs measuring, not assuming.
- CONFIRM: index 2 of `vpnc_dev_policy_list` is empty on every record seen; purpose unknown. Write it empty and match the WebUI.
- DELETE: `.claude\testing\2026-09-07_pristine-lan-reference.md` once device assignment has shipped.
- commit.
