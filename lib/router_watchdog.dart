...
class WatchdogStatus {
  ...
  /// The app version stamped into the DEPLOYED script's header, or null when there is no script
  /// or its header is not one of ours. Updating the app does not update the router - the script
  /// only changes when a watchdog is deployed - so the two drift silently, and a fix shipped in
  /// the app can look like it did not work. [scriptIsCurrent] is the question worth asking.
  final String? scriptVersion;

  const WatchdogStatus({required this.isEnabled, this.lastSuccessfulPing, this.scriptVersion});

  /// False only when both versions are known AND differ. An unknown version is not a mismatch:
  /// tests and older deployments have no marker, and crying wolf there would train it to be
  /// ignored.
  bool get scriptIsCurrent =>
      scriptVersion == null || appVersionLabel.isEmpty || scriptVersion == appVersionLabel;
}

/// Every NVRAM key the app writes, unset in one round trip.
///
/// The five globals plus ten per-slot watchdog keys for each of wgc1..wgc5. `nvram unset` on a key
/// that was never set is not an error, so this needs no probing first.
///
...
/// Finds the header line of whichever watchdog script is on the router, for the ABOUT screen.
///
/// One command rather than a probe per slot, and `grep -h` so the filename does not come back
/// with it. Any deployed script answers the question - they are all written from the same
/// template by the same build - so the first match is enough.
const String kDeployedScriptHeaderCommand =
    'grep -h "auto-generated by cfg-pia-wg" $kRouterAppDir/watchdog_wgc*.sh 2>/dev/null | head -1';

/// Pulls the version out of a deployed script's header line, which reads
/// `# watchdog_wgcN.sh - auto-generated by cfg-pia-wg v0.8.46 build 416; *do* *not* edit.`
///
/// Returns null for anything else, including the pre-415 scripts that carry no version at all.
String? parseScriptVersion(String headerLine) =>
    RegExp(r'auto-generated by cfg-pia-wg (v[0-9][^;]*?)\s*;').firstMatch(headerLine)?.group(1);
...
class RouterWatchdog {
  ...
  /// Polls until `wgcN` is actually UP, or the attempts run out.
  ///
  /// [kUpInterfacesCommand], not `wg show interfaces` - the latter lists configured devices and
  ...
  /// A line that appears in the app's copy of an init script and never in the router's own, so a
  /// backup is only ever taken of the ORIGINAL.
  /// The header BOTH replacement scripts carry, so "is this ours?" is one question with one
  /// answer. Added to `S50downloadmaster` in 428: it previously had to be recognised by its
  /// REPLACEMENT markers, which meant the uninstall could not tell the router's own script from
  /// ours after a restore and deleted the original on a second run. Reported 2026-09-10.
  static const String _kOurScriptSignature = 'auto-generated by cfg-pia-wg';

  /// Copies an init script to `<path>.old` before the app replaces it, once and only once.
  ///
  /// Three guards, and each one matters. The file has to EXIST - on a router where it does not,
  /// there is nothing to preserve and an empty `.old` would be restored over a working setup by
  /// the uninstall feature. `.old` must not already exist, or a second deploy would overwrite a
  /// good backup. And the file must not already be OURS, or the backup would preserve the app's
  /// own script as though it were the router's; if `.old` is missing at that point the original is
  /// already gone, and writing a false one is worse than admitting there is none.
  ///
  /// Best-effort. A router that will not take the copy still gets a working watchdog, and failing
  /// the deploy over a backup would trade the feature for the ability to undo it.
  Future<void> _backupOriginal(String path, {required String ours}) async {
    final backup = originalScriptBackupPath(path);
    final result = await _read("[ -f '$path' ] && [ ! -f '$backup' ] && "
        "! grep -q '$ours' '$path' && cp -p '$path' '$backup' && echo SAVED || echo SKIP");
    if (result.contains('SAVED')) {
      onLog?.call('Original ${path.split('/').last} saved as ${backup.split('/').last}.', isSuccess: true);
      await _logRouter('Original ${path.split('/').last} saved as $backup');
    }
  }

  /// Puts the router back the way the app found it, and reports what it actually did.
  ///
  /// Three steps, in this order: restore each init script the app took over from its `.old` copy,
  /// then remove the app's own directory. Restoring first means that if the directory removal
  /// fails, the boot scripts are already back - the reverse order could leave a router with no
  /// init script AND the app's files still on it.
  ///
  /// A missing `.old` is not an error. It means the original was already gone before the app ever
  /// wrote a backup - true for anyone who installed before 419 - so the app's own copy is removed
  /// instead of being left behind pretending to be the router's. Each step reports which of the
  /// two happened, because "restored yours" and "removed ours" are different outcomes and the user
  /// is entitled to know which they got.
  ///
  /// Deliberately does NOT touch cron entries, NVRAM or the tunnels. Those belong to the watchdog
  /// and the slots, which have their own DELETE; an uninstall that silently tore down a working VPN
  ...
  Future<List<String>> uninstallFromRouter() => _guard('uninstall from router', () async {
        ...
        for (final path in [kS50Path, kS50LighttpdPath]) {
          ...
          final backup = originalScriptBackupPath(path);
          // The `grep` is the whole fix for the 425 bug. Uninstall run twice deleted the ROUTER'S
          // OWN scripts the second time: the first run restored them, and the second saw a file it
          // did not recognise and removed it. Never touch a file without our header on it.
          final result = await _read("[ -f '$backup' ] && mv -f '$backup' '$path' && echo RESTORED || "
              "{ [ -f '$path' ] && grep -q '$_kOurScriptSignature' '$path' && rm -f '$path' && echo REMOVED || "
              "{ [ -f '$path' ] && echo NOTOURS || echo ABSENT; }; }");
          if (result.contains('RESTORED')) {
            done.add('Restored the original $name');
          } else if (result.contains('REMOVED')) {
            done.add("Removed the app's $name - no original was saved to put back");
          } else if (result.contains('NOTOURS')) {
            done.add('Left $name alone - it is not ours');
          } else {
        ...
        await _logRouter('cfg-pia-wg uninstalled from this router');
        return done;
      });

  /// Replaces S50asuslighttpd with a stub that returns immediately on any trigger.
  ///
  /// The stock script runs at boot and again on every VPN up or down, and it sleeps - which stalls
  /// the boot outright when the router starts with a VPN already enabled. Nothing the app needs is
  /// in it. Written on every deploy rather than once, so a firmware update that restores the
  /// original is undone the next time a watchdog is deployed.
  Future<void> _writeLighttpdStub() async {
    await _backupOriginal(kS50LighttpdPath, ours: _kOurScriptSignature);
    await _writeFile(kS50LighttpdPath, kS50AsusLighttpdTemplate, what: 'Boot stub', mode: '700');
  }
  ...
  Future<WatchdogStatus> getWatchdogStatus(int slot) async {
    ...
    if (cronEnabled) {
      final line = 'wgc$slot watchdog script ${scriptVersion ?? 'version unknown'}, '
          'app ${appVersionLabel.isEmpty ? 'version unknown' : appVersionLabel}'
          '${status.scriptIsCurrent ? '' : ' - redeploy the watchdog to update it'}';
      onLog?.call('$line.');
      await _logRouter(line);
    }
    return status;
  }

  /// The version stamped into the deployed watchdog script, or null when none is deployed.
  ///
...
// curl command line executed (!) and there is no way to disable this, so after every
// curl invocation, the curllst log file is flushed with "echo -n > /jffs/curllst" :)
// The same file records each caller's process ancestry, and that is what exposed the caller
// check curl enforces - see the detach guard at the top of the template.
//
const String _kWatchdogScriptTemplate = r'''#!/bin/sh
# watchdog_wgc__SLOT__.sh - auto-generated by cfg-pia-wg __APPVER__; *do* *not* edit.
...
if [ "$RUNMODE" = "detached" ]; then
  # Waiting for the parent to exit is what reparents us to init, and is load-bearing: a curl that
  # runs before the handover is rejected exactly as before. Downstream still reads "cron".
  DN=0
  while [ "$(awk '/^PPid:/{print $2}' /proc/$$/status 2>/dev/null)" != "1" ] && [ "$DN" -lt 10 ]; do
    sleep 1
    DN=$((DN + 1))
  done
  RUNMODE="cron"
...
abort() {
  ...
  # /jffs/curllst. Success paths flush it; without this one a failure left the password on flash.
  echo -n > /jffs/curllst
...
WRITEOUT="$(cat "$TMPHTTP" 2>/dev/null)"
HTTP="$(echo "$WRITEOUT" | cut -d' ' -f1)"
...
# Kept, not deleted, when the fetch fails: these files are the whole evidence base for a failure
# that only happens under cron and has never been reproduced by hand.
[ -n "$HTTP" ] && [ "$HTTP" != "000" ] && rm -f "$TMPHTTP"
TOKEN=""
