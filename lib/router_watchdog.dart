...
class RouterWatchdog {
  ...
  Future<void> _ensureServicesStart(int slot, int intervalMin) async {
    if (isStockFirmware) {
      await _backupOriginal(kS50Path, ours: _kStartMarkerSignature);
  ...
  static const String _kStartMarkerSignature = 'REPLACEMENT START';
  static const String _kLighttpdStubSignature = 'auto-generated by cfg-pia-wg';
  ...
  Future<List<String>> uninstallFromRouter() => _guard('uninstall from router', () async {
        ...
        for (final path in [kS50Path, kS50LighttpdPath]) {
          ...
          final result = await _read("[ -f '$backup' ] && mv -f '$backup' '$path' && echo RESTORED || "
              "{ [ -f '$path' ] && rm -f '$path' && echo REMOVED || echo ABSENT; }");
  ...
  Future<void> _writeLighttpdStub() async {
    await _backupOriginal(kS50LighttpdPath, ours: _kLighttpdStubSignature);
