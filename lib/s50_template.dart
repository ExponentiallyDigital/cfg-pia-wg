...
//
// [kS50DownloadmasterTemplate] mirrors ./scripts/S50downloadmaster-TEMPLATE.sh verbatim, LF line
// endings included: a CR would make the router's kernel fail to exec the shebang.
// test/unit/s50_template_test.dart fails if the two drift apart.
...
/// The replacement for `/opt/etc/init.d/S50asuslighttpd`, mirroring
/// `./scripts/S50asuslighttpd-TEMPLATE.sh` verbatim (LF only, as above).
///
/// The stock script is called at boot AND on every VPN up or down, and it carries sleeps -
/// which is what stalls the boot when the router starts with a VPN already enabled, and what
/// makes every tunnel change cost more than it should. Nothing in it is wanted here, so it is
/// replaced by a stub that returns immediately whatever trigger it is given. The original is
/// kept as `S50asuslighttpd.old` and the uninstall feature puts it back.
const String kS50AsusLighttpdTemplate = r'''#!/bin/sh
...
# S50downloadmaster - auto-generated by cfg-pia-wg; *do* *not* edit.
...
[ "$1" = "start" ] || exit 0
  ...
  # ********** REPLACEMENT END **********
  # Put the fail-closed guard back: a reboot clears its rules (ID-213).
