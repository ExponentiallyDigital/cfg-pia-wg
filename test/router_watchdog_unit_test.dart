...
void main() {
  group('validation helpers', () {
    ...
    test('parseScriptVersion reads the version out of a deployed script header', () {
      const header = '# watchdog_wgc1.sh - auto-generated by cfg-pia-wg v0.8.46 build 416; *do* *not* edit.';
      expect(parseScriptVersion(header), 'v0.8.46 build 416');
      // Scripts deployed before the marker existed, and anything that is not one of ours.
      expect(parseScriptVersion('# watchdog_wgc1.sh - auto-generated by cfg-pia-wg; do not edit.'), isNull);
      expect(parseScriptVersion('#!/bin/sh'), isNull);
      expect(parseScriptVersion(''), isNull);
    });

    test('scriptIsCurrent only calls a mismatch when both versions are known', () {
      final saved = appVersionLabel;
      addTearDown(() => appVersionLabel = saved);
      appVersionLabel = 'v0.8.46 build 416';

      expect(const WatchdogStatus(isEnabled: true, scriptVersion: 'v0.8.46 build 416').scriptIsCurrent, isTrue);
      expect(const WatchdogStatus(isEnabled: true, scriptVersion: 'v0.8.45 build 415').scriptIsCurrent, isFalse);
      // An unknown version is not a mismatch - an alert nobody can act on trains people to ignore
      // the ones they can.
      expect(const WatchdogStatus(isEnabled: true).scriptIsCurrent, isTrue);

      appVersionLabel = '';
      expect(const WatchdogStatus(isEnabled: true, scriptVersion: 'v0.8.45 build 415').scriptIsCurrent, isTrue);
    });
  ...
  group('buildWatchdogScript', () {
    ...
    // ASUS's /usr/sbin/curl refuses to run with crond in its process ancestry - it exits 0 having
    // written no status, no body and no stderr, and logs "Invalid caller(crond)". Measured
    // 2026-09-09: the same command returned nothing from cron and "200 exit=0" once reparented to
    // init. Without the detach the watchdog cannot fetch a PIA token from cron, so it can never
    // recover a tunnel by itself - which is the outage this group exists to prevent recurring.
    test('a cron run re-execs itself detached and does no work in the first process', () {
      final s = buildWatchdogScript(_valid(slot: 1));
      expect(s, contains(r'"$0" detached >/dev/null 2>&1 &'));

      // The first process must exit immediately: anything it did before exiting would run twice.
      final guard = s.indexOf(r'"$0" detached');
      expect(guard, greaterThan(0));
      expect(s.substring(guard, s.indexOf('\n', s.indexOf('exit 0', guard))), contains('exit 0'));

      // And it must happen before anything that calls curl, or the rejection still applies.
      expect(guard, lessThan(s.indexOf('CACERT_URL')));
      expect(guard, lessThan(s.indexOf('TOKEN_URL')));
    });

    test('the detached copy waits to be reparented to init, then continues as a cron run', () {
      final s = buildWatchdogScript(_valid(slot: 1));
      expect(s, contains(r'if [ "$RUNMODE" = "detached" ]; then'));
      // Reparenting is what removes crond from the ancestry, so the wait is load-bearing rather
      // than defensive: a curl that runs before the handover is rejected exactly as before.
      expect(s, contains(r"awk '/^PPid:/{print $2}' /proc/$$/status"));
      // Everything downstream branches on cron vs deploy and must not learn a third mode.
      expect(s, contains('RUNMODE="cron"'));
    });

    test('deploy runs are not detached, because SSH callers are not rejected', () {
      final s = buildWatchdogScript(_valid(slot: 1));
      final guard = s.indexOf(r'"$0" detached');
      // The guard is entered only for cron. A deploy run is the app running the script over SSH,
      // where the parent is dropbear and curl works; detaching it would also lose its output.
      expect(s.substring(0, guard), contains(r'if [ "$RUNMODE" = "cron" ]; then'));
    });

    // The app updates from the store; the deployed script only changes when a watchdog is
    // deployed. Without a version in both logs, a user running a build whose fix has never
    // reached their router has no way to tell - which is exactly how the 415 interface fix was
    // tested against a script from two builds earlier.
    test('every run names the script version in both logs', () {
      final s = buildWatchdogScript(_valid(slot: 1));
      expect(s, contains(r'log "Watchdog started for $IFACE${APPVER:+ [script $APPVER]}"'));
      // log() writes to the log file and to syslog via logger, so one line covers both.
    ...
    test('abort empties curllst so a failed token fetch leaves no PIA password on flash', () {
      ...
      // world-readable /jffs/curllst. The success paths already flushed it; abort did not, so a
      // failure left the password there until the next successful reconfigure.
      final abort = s.indexOf('abort() {');
      expect(abort, greaterThan(0));
      final body = s.substring(abort, s.indexOf('\n}\n', abort));
      expect(body, contains('echo -n > /jffs/curllst'));
    });
    ...
    test('reports the HTTP status and curl exit code when the token request fails', () {
      ...
      // 2026-09-09: %{exitcode}, %{errormsg} and %{num_connects} were added, because five
      // failures reported exit 0 with no status, no body and no stderr - which curl should not be
      // able to produce. Asking curl what happened beats inferring it from what it did not say.
      expect(s, contains(r'%{http_code} exit=%{exitcode}'));
      expect(s, contains(r'err=%{errormsg}'), reason: 'curl 7.75+ can name its own failure');
      ...
      // The retry and the kept evidence, both added 2026-09-09. A failure that only happens under
      // cron and has never been reproduced by hand has to leave something behind to look at.
      expect(s, contains('retrying once in 3s'), reason: 'one retry before giving up');
      // The deployed script must be identifiable from the router log alone. Five failures were
      // diagnosed against source that may not have been what actually ran (2026-09-09).
      expect(s, contains(r'reconfiguring (attempt #$CNT)${APPVER:+ [script $APPVER]}'));
      expect(s, contains('auto-generated by cfg-pia-wg'));
      expect(s, contains(r'[ -n "$HTTP" ] && [ "$HTTP" != "000" ] && rm -f "$TMPHTTP"'),
          reason: 'the diagnostic files survive a failure');
