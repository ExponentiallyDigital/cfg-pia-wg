...
void main() {
  ...
  group('the lighttpd stub', () {
    test('matches ./scripts/S50asuslighttpd-TEMPLATE.sh', () {
      final repo = File('scripts/S50asuslighttpd-TEMPLATE.sh').readAsStringSync();
      expect(kS50AsusLighttpdTemplate, repo.replaceAll('\r\n', '\n'),
          reason: 'lib/s50_template.dart has drifted from scripts/S50asuslighttpd-TEMPLATE.sh');
    });

    test('ships LF-only, whatever the repo copy uses', () {
      expect(kS50AsusLighttpdTemplate.contains('\r'), isFalse);
    });

    // The stock script sleeps, and runs on every VPN up or down as well as at boot - which is what
    // stalls a boot that starts with a VPN enabled. The replacement must do nothing at all.
    test('returns immediately and does nothing else', () {
      expect(kS50AsusLighttpdTemplate, startsWith('#!/bin/sh\n'));
      expect(kS50AsusLighttpdTemplate.trimRight(), endsWith('exit 0'));
      for (final line in kS50AsusLighttpdTemplate.split('\n')) {
        final code = line.trim();
        if (code.isEmpty || code.startsWith('#')) continue;
        expect(code, anyOf('#!/bin/sh', 'exit 0'), reason: 'the stub must contain no logic: "$code"');
      }
    });

    // Without this the backup guard cannot tell the app's copy from the router's, and a second
    // deploy would save OUR stub as though it were the original.
    test('carries the marker the backup guard looks for', () {
      expect(kS50AsusLighttpdTemplate, contains('auto-generated by cfg-pia-wg'));
    });
  });
