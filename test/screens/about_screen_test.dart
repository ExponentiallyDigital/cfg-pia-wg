...
void main() {
  ...
  test('licence status reads homegrown without a store key, otherwise licensed or unlicenced', () {
    expect(licenceStatus(purchasingAvailable: false, unlocked: true), 'homegrown');
    expect(licenceStatus(purchasingAvailable: true, unlocked: true), 'licensed');
    expect(licenceStatus(purchasingAvailable: true, unlocked: false), 'unlicenced');
  });

  test('router firmware status: type and version, the type alone, or not detected', () {
    resetRouterFirmware();
    expect(routerFirmwareStatus(type: 'stock', version: '3.0.0.4.388_25127'), 'stock 3.0.0.4.388_25127');
    expect(routerFirmwareStatus(), 'not detected this session');
    setRouterFirmware(RouterFirmware.merlin);
    addTearDown(resetRouterFirmware);
    expect(routerFirmwareStatus(), 'Merlin', reason: 'the session knows the type even before the router is read');
  });
  ...
  group('prefilled bug report', () {
    ...
    // A local holding the firmware TYPE was called `firmware`, and shadowed the version passed in.
    test('carries the firmware type AND the version read from the router', () {
      resetRouterFirmware();
    ...
    test('carries the licence status', () {
      final body = Uri.parse(bugReportUrl(null, licence: 'homegrown')).queryParameters['body']!;
      expect(body, contains('License status: homegrown'));
    });
  ...
  testWidgets('renders the build metadata supplied by the platform channel', (tester) async {
    ...
    // A test build has no store key, so it is a homegrown copy.
    expect(find.textContaining('License status: homegrown', findRichText: true), findsOneWidget);
  ...
  group('deployed watchdog script version', () {
    ...
    // The app updates from the store and the script only on a deploy, so they drift apart silently.
    testWidgets('an older deployed script is amber and offers a redeploy; a current one offers nothing', (tester) async {
      _mockChannel(tester, (call) async => _hostReply);
      appVersionLabel = 'v0.8.76 build 446';
      addTearDown(() => appVersionLabel = '');

      Future<void> pumpWith(String deployed) async {
        final ssh = RecordingSSHClient(
          responder: (_) => '# watchdog_wgc1.sh - auto-generated by cfg-pia-wg $deployed; *do* *not* edit.',
        );
        final c = _quietController()
          ..routerIp = '192.168.1.1'
          ..sshUsername = 'admin'
      ...
      await pumpWith('v0.8.49 build 419');
      ...
      expect(find.byKey(const Key('about_redeploy_scripts')), findsNothing);
    });

    // ID-157: a Play build installed over a newer sideloaded one leaves the router's script AHEAD of
    // the app. Then the app is the one behind: its version goes red, the script's stays amber.
    testWidgets('a script newer than the app turns the app version red and keeps the update offer', (tester) async {
      _mockChannel(tester, (call) async => _hostReply);
      appVersionLabel = 'v0.8.87 build 457';
      addTearDown(() => appVersionLabel = '');
      final ssh = RecordingSSHClient(
        responder: (_) => '# watchdog_wgc1.sh - auto-generated by cfg-pia-wg v0.8.88 build 458; *do* *not* edit.',
      );
      final c = _quietController()
        ..routerIp = '192.168.1.1'
        ..sshUsername = 'admin'
      ...
      for (final t in tester.widgetList<Text>(find.byType(Text))) {
        t.textSpan?.visitChildren((span) {
          if (span is TextSpan && span.text != null) colours[span.text!] = span.style?.color;
          return true;
        });
      }
      expect(colours['v0.8.88 build 458'], kWarn);
      expect(colours.entries.where((e) => e.value == kError && e.key.startsWith('v')), hasLength(1),
          reason: "the app's own version, in the first line");
      expect(find.byKey(const Key('about_redeploy_scripts')), findsOneWidget);
    });

    test('buildNumberOf reads the build out of a version label', () {
      expect(buildNumberOf('v0.8.88 build 458'), 458);
      expect(buildNumberOf('not deployed'), isNull);
    });
